...
def _extract_printed_page_number(lines: list[str]) -> int | None:
    for line in lines[:8]:
        stripped = line.strip()
        if stripped.isdigit():
            return int(stripped)
...
def _extract_contents_entries(lines: list[str]) -> list[JsonDict]:
        ...
        entries.append(
            {
                ...
                "printed_page_number": int(match.group("page")),
            }
        )
    return entries
...
def _extract_pdf_pages(path: Path, *, doc_key: str, doc_label: str) -> list[JsonDict]:
    ...
    for idx in range(doc.page_count):
        page_number = idx + 1
        raw_lines = [line.strip() for line in doc.load_page(idx).get_text("text").splitlines() if line.strip()]
        text = " ".join(raw_lines)
        lowered = text.lower()
        pages.append(
            {
                ...
                "printed_page_number": _extract_printed_page_number(raw_lines),
                "doc_key": doc_key,
                "doc_label": doc_label,
                "text_preview": text[:400],
                "contents_entries": _extract_contents_entries(raw_lines),
                "flags": {
