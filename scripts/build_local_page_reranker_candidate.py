...
try:
    ...
    from probe_local_embedding_relevance import _extract_page_text, _parse_page_id
    ...
    from scripts.probe_local_embedding_relevance import _extract_page_text, _parse_page_id

from rag_challenge.core.local_page_reranker import LocalPageReranker, PageRerankScore
...
def _baseline_doc_pages(page_ids: list[str]) -> dict[str, list[int]]:
    ...
    for page_id in page_ids:
        ...
        if page_number not in out[doc_id]:
            out[doc_id].append(page_number)
    for doc_id in out:
        out[doc_id].sort()
    return out
...
def _candidate_pages_for_doc(
    *,
    ...
    def push(page_number: int) -> None:
        if page_number < 1 or page_number > page_count or page_number in seen:
            return
        seen.add(page_number)
        ranked_pages.append(page_number)

    for page_number in baseline_pages:
        push(page_number)
    for page_number in baseline_pages:
        for delta in range(1, max(0, neighbor_radius) + 1):
            push(page_number - delta)
            push(page_number + delta)
    if include_page_one:
        push(1)
    if include_page_two:
    ...
    if max_pages_per_doc > 0:
        ranked_pages = ranked_pages[:max_pages_per_doc]
    return [f"{doc_id}_{page_number}" for page_number in ranked_pages]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a support-only candidate by reranking bounded page candidates with a local cross-encoder.")
