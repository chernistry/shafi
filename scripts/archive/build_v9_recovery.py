#!/usr/bin/env python3
"""V9 Recovery: Re-run nopg questions with free_text routing override.

Same strategy as V8.1 recovery (build_v8_1_recovery.py), applied to V9 eval results.
...
Strategy:
  - Load V9 full 900Q eval results
  - For each 0-page question, re-run with answer_type=free_text on V9 server (port 8003)
  - Extract correct-type answer from free_text response
  - Update eval results for recovered questions
  - Save as tzuf_v9_1_full900.json and build new submission

Expected: similar recovery to V8.1 (~10 questions recovered).
"""
...
V9_RESULTS = REPO / "data" / "tzuf_v9_full900.json"
...
def main() -> None:
    if not V9_RESULTS.exists():
        print(f"ERROR: V9 results not found at {V9_RESULTS}")
        print("Run tzuf_v9_full900.py first.")
        return

    data = json.loads(V9_RESULTS.read_text())
    ...
    print(f"V9 nopg questions: {len(nopg)} total")
    ...
    output = dict(data)
    output["eval_id"] = "tzuf-v9-1-recovery"
    ...
    output["g_proxy_delta_vs_v9"] = round(g - data.get("g_proxy", 0.9844), 4)
    output["ttft_avg_ms"] = round(ttft_avg, 1)
    output["results"] = valid

    OUTPUT.write_text(json.dumps(output, indent=2))
    print(f"\n{'='*60}")
    print(f"V9.1 RECOVERY COMPLETE — {recovered} questions recovered")
