#!/usr/bin/env python3
"""TZUF V8: Full 900Q private eval — G-guard + boolean regex (zerank-2).

Code: HEAD=0176381 (G-guard + skip_rerank_strict_doc_ref + boolean regex + FlashRank adapter)
Config: .env.local (same as V7) — RERANK_PROVIDER_MODE defaults to api = zerank-2
Server: port 8002 (V8 server, separate from V7 at port 8000)

Changes vs V7 (843ec40):
  - df0676e: G-guard (db_lookup no-page fallback), skip_rerank_strict_doc_ref flag, boolean regex
  - 0176381: FlashRank adapter (NOT active in default config)
...
OUTPUT = REPO / "data" / "tzuf_v8_full900.json"
CHECKPOINT = REPO / "data" / "tzuf_v8_checkpoint.jsonl"
...
def load_checkpoint() -> dict[str, dict]:
    """Load previously answered questions from checkpoint."""
...
def main(resume: bool = False) -> None:
    ...
    if resume:
        ...
        print(f"Resuming V8 eval — {len(done)} already done, {total - len(done)} remaining")
    else:
        if CHECKPOINT.exists():
            CHECKPOINT.unlink()
        print(f"Starting V8 fresh eval — {total} questions")
    ...
    # Pre-populate by_type from resumed results
    for r in results:
    ...
    print(f"\n{'='*60}")
    print(f"V8 EVAL COMPLETE — {len(valid)}/{total} answered (errors={errors})")
    print(f"Null:     {null_count} ({null_count / total * 100:.1f}%) [V7=13 (1.4%)]")
    print(f"No-pages: {no_pg_count} ({no_pg_count / total * 100:.1f}%) [V7=17 (1.9%)]")
    print(f"G_proxy:  {g_proxy:.4f} [V7=0.9811]")
    print(f"TTFT avg: {ttft_avg:.0f}ms [V7=2130ms]")
    print("\nBy type:")
    ...
    output_data = {
        "eval_id": "tzuf-v8-full900",
        "code_version": "0176381",
        ...
        "g_proxy_v7_baseline": 0.9811,
        "g_proxy_delta": round(g_proxy - 0.9811, 4),
