#!/usr/bin/env python3
"""TZUF V12: Full 900Q private eval — ALL 16 fixes since V9.1.
...
OUTPUT = REPO / "data" / "tzuf_v12_full900.json"
CHECKPOINT = REPO / "data" / "tzuf_v12_checkpoint.jsonl"
...
def main(resume: bool = False) -> None:
    ...
    if resume:
        ...
        print(f"Resuming V12 eval — {len(done)} already done, {total - len(done)} remaining")
    else:
        if CHECKPOINT.exists():
            CHECKPOINT.unlink()
        print(f"Starting V12 fresh eval — {total} questions")
        print(f"Server: {SERVER_URL}")
        print(f"Head: 3e28cf4 (ALL 16 fixes since V9.1)")
    ...
    print(f"\n{'='*60}")
    print(f"V12 EVAL COMPLETE — {len(valid)}/{total} answered (errors={errors})")
    ...
    output_data = {
        "eval_id": "tzuf-v12-full900",
        "code_version": "3e28cf4",
