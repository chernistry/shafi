#!/usr/bin/env python3
"""TZUF V15: Full 900Q private eval — ALL 16 fixes since V9.1.

Code: HEAD=057048c
Config: .env.local (zerank-2 + DB_ANSWERER + EQA + citation_hop + predicted_outputs)
Server: port 8000 (restarted on HEAD with all fixes)
...
OUTPUT = REPO / "data" / "tzuf_v15_full900.json"
CHECKPOINT = REPO / "data" / "tzuf_v15_checkpoint.jsonl"
SERVER_URL = "http://localhost:8000/query"
TIMEOUT = 120.0

V9_1_BASELINE_G = 0.9956
V9_1_BASELINE_TTFT = 1242.0
...
def compute_f_coefficient(ttft_ms: float) -> float:
    if ttft_ms < 1000:
        return 1.05
    elif ttft_ms < 2000:
        return 1.02
        ...
        return 0.99 - (ttft_ms - 3000) * 0.14 / 2000
    else:
        return 0.85
...
def main(resume: bool = False) -> None:
    ...
    if resume:
        ...
        print(f"Resuming V15 eval — {len(done)} already done, {total - len(done)} remaining")
    else:
        if CHECKPOINT.exists():
            CHECKPOINT.unlink()
        print(f"Starting V15 fresh eval — {total} questions")
        print(f"Server: {SERVER_URL}")
        print(f"Head: 057048c (ALL 16 fixes since V9.1)")
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        for i, q in enumerate(qs, 1):
            ...
            if i % 50 == 0:
                ...
                f_coeff = compute_f_coefficient(ttft_avg)
                print(
                    f"\n  === V15 PROGRESS [{i}/{total}]: "
                    f"null={null_count} no_pages={no_pg_count} G_proxy={g_proxy:.4f} "
                    f"TTFT_avg={ttft_avg:.0f}ms F={f_coeff:.4f} [V9.1={V9_1_BASELINE_TTFT:.0f}ms] ===\n"
                )
    ...
    f_values = [compute_f_coefficient(t) for t in ttft_all]
    f_mean = sum(f_values) / len(f_values) if f_values else 0.0

    print(f"\n{'='*60}")
    print(f"V15 EVAL COMPLETE — {len(valid)}/{total} answered (errors={errors})")
    ...
    print(f"F (avg):  {f_coeff:.4f}")
    print(f"F (mean): {f_mean:.4f}")
    print("\nBy type:")
    ...
    output_data = {
        "eval_id": "tzuf-v15-full900",
        "code_version": "057048c",
        "config": ".env.local (ALL 16 fixes + DB_ANSWERER + EQA + citation_hop)",
        "total": len(valid),
        "errors": errors,
        ...
        "f_coefficient_avg": round(f_coeff, 4),
        "f_coefficient_mean": round(f_mean, 4),
        "by_type": {
            t: {
                "count": d["count"],
                "null_count": d["null"],
