...
from shafi.ingestion.pipeline import IngestionPipeline, IngestionStats
...
async def _run_proxy(*, fixture_dir: Path, target_docs: int) -> JsonDict:
    ...
    with tempfile.TemporaryDirectory(prefix="ingestion-stress-proxy-") as tmp_dir:
        ...
        with patch("shafi.ingestion.pipeline.get_settings", return_value=_pipeline_settings_stub(manifest_dir)):
