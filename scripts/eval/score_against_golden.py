...
def _extract_used_page_ids(telemetry: dict[str, Any]) -> list[str]:
    # Mirrors select_submission_used_pages (submission/common.py) and
    # select_used_pages (eval/sources.py) to match platform scoring semantics.
    # Fallback chain: used_page_ids → cited_page_ids → retrieved_chunk_ids.
    # used_page_ids = sidecar evidence selection (what submission sends to platform).
    # cited_page_ids = LLM-cited pages (fallback only — may differ from sidecar).
