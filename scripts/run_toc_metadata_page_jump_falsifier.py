...
_FAMILY_TOC_PATTERNS: dict[str, tuple[re.Pattern[str], ...]] = {
    "citation_title": (re.compile(r"^title(?: and repeal)?$", re.IGNORECASE),),
    "title_page": (re.compile(r"^title(?: and repeal)?$", re.IGNORECASE),),
    ...
    "schedule": (
        re.compile(r"^schedule(?:s)?(?:\s+\d+)?$", re.IGNORECASE),
...
def _page_score(page: JsonDict, *, family: str) -> float:
    ...
    if family == "no_waiver" and bool(flags.get("has_no_waiver")):
        score += 14.0
    if family == "schedule" and bool(flags.get("has_schedule")):
        score += 14.0
    score += max(0.0, 3.0 - (page_number * 0.1))
    return score


def _sorted_pages(case: JsonDict, *, family: str) -> list[JsonDict]:
    pages = _coerce_dict_list(case.get("pages"))
    return sorted(
        pages,
        key=lambda page: (
            -_page_score(page, family=family),
            _coerce_int(page.get("page_number"), default=9999),
            str(page.get("page_id") or ""),
        ),
    )


def _select_current_candidate(case: JsonDict) -> list[str]:
    pages = _coerce_dict_list(case.get("pages"))
    ...
    ranked = _sorted_pages(case, family=family)
    selected: list[str] = []
...
def _find_cover_page(pages: list[JsonDict]) -> str:
    return next((str(page["page_id"]) for page in pages if bool(cast("JsonDict", page["flags"]).get("is_cover_page"))), "")


def _find_contents_page(pages: list[JsonDict]) -> JsonDict | None:
    return next((page for page in pages if bool(cast("JsonDict", page["flags"]).get("is_contents_page"))), None)


def _match_toc_entry(contents_page: JsonDict, *, family: str) -> JsonDict | None:
    patterns = _FAMILY_TOC_PATTERNS.get(family, ())
    if not patterns:
        return None
    for entry in _coerce_dict_list(contents_page.get("contents_entries")):
...
def _family_flag_matches(page: JsonDict, *, family: str) -> bool:
    flags = cast("JsonDict", page["flags"])
    return {
        "citation_title": bool(flags.get("has_citation_title")),
        "title_page": bool(flags.get("has_citation_title")) or bool(flags.get("has_law_number")),
        "who_made": bool(flags.get("has_legislative_authority")),
        "enactment": bool(flags.get("has_enactment")),
        "commencement": bool(flags.get("has_commencement")),
        "administration": bool(flags.get("has_administration")),
        "no_waiver": bool(flags.get("has_no_waiver")),
        "schedule": bool(flags.get("has_schedule")),
    }.get(family, False)


def _find_best_destination_page(pages: list[JsonDict], *, printed_page_number: int, family: str) -> JsonDict | None:
    matching = [
        page
        for page in pages
        if _coerce_int(page.get("printed_page_number"), default=-1) in {printed_page_number, printed_page_number + 1}
        and not bool(cast("JsonDict", page["flags"]).get("is_contents_page"))
    ]
    if not matching:
        return None
    return sorted(
        matching,
        key=lambda page: (
            0 if _family_flag_matches(page, family=family) else 1,
            -_page_score(page, family=family),
            _coerce_int(page.get("page_number"), default=9999),
        ),
    )[0]
...
def _select_toc_candidate(case: JsonDict) -> JsonDict:
    pages = _coerce_dict_list(case.get("pages"))
    family = str(case.get("family") or "")
    if not pages:
        return {"selected_pages": [], "matched_contents_label": None, "target_printed_page_number": None}

    cover_page = _find_cover_page(pages)
    current_candidate = _select_current_candidate(case)
    selected: list[str] = []
    if cover_page and family in {
        "citation_title",
        "who_made",
    ...
    contents_page = _find_contents_page(pages)
    matched_label: str | None = None
    target_printed_page_number: int | None = None
    if contents_page is not None:
        entry = _match_toc_entry(contents_page, family=family)
        if entry is not None:
            matched_label = str(entry.get("label") or "")
            target_printed_page_number = _coerce_int(entry.get("printed_page_number"), default=-1)
            destination_page = _find_best_destination_page(
                pages,
                printed_page_number=target_printed_page_number,
                family=family,
            )
            if destination_page is not None:
                page_id = str(destination_page["page_id"])
                if page_id not in selected:
                    selected.append(page_id)

    for page_id in current_candidate:
        if page_id in selected:
            continue
        selected.append(page_id)
        if len(selected) >= 2:
    ...
    return {
        "selected_pages": selected[:2],
        "matched_contents_label": matched_label,
        "target_printed_page_number": target_printed_page_number,
    }
...
def run_falsifier(pack: JsonDict) -> JsonDict:
    ...
    for case in cases:
        ...
        baseline = _select_current_candidate(case)
        toc_candidate = _select_toc_candidate(case)
        candidate_pages = [str(page_id) for page_id in cast("list[object]", toc_candidate["selected_pages"])]
        baseline_score = _f_beta(set(baseline), gold_pages)
        candidate_score = _f_beta(set(candidate_pages), gold_pages)
        is_improved = candidate_score > baseline_score + 1e-9
        if is_improved:
            improved += 1
        results.append(
            {
                ...
                "matched_contents_label": toc_candidate["matched_contents_label"],
                "target_printed_page_number": toc_candidate["target_printed_page_number"],
...
def _render_markdown(payload: JsonDict) -> str:
    ...
    lines = [
        "# TOC-Aware Metadata Page-Jump Falsifier",
        "",
    ...
    for row in improved_rows:
        lines.extend(
            [
                ...
                f"- matched_contents_label: `{row['matched_contents_label']}`",
                f"- target_printed_page_number: `{row['target_printed_page_number']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the TOC-aware metadata page-jump falsifier.")
