...
async def test_ingestion_pipeline_with_real_fixtures_and_mocked_external_services() -> None:
    ...
    with TemporaryDirectory() as manifest_tmp_dir:
        ...
        with (
            patch("rag_challenge.ingestion.pipeline.get_settings", return_value=settings),
            patch("rag_challenge.ingestion.chunker.get_settings", return_value=_chunker_settings_stub()),
        ):
...
async def test_qdrant_smoke_optional_skips_if_unavailable() -> None:
    ...
    with patch("rag_challenge.core.qdrant.get_settings", return_value=settings):
        store = QdrantStore()
        try:
