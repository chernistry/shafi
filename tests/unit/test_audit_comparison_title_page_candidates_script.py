...
def test_audit_comparison_title_page_candidates_detects_monetary_claim_compare(tmp_path: Path) -> None:
    questions_path = tmp_path / "questions.json"
    truth_path = tmp_path / "truth.json"
    baseline_raw_path = tmp_path / "baseline_raw.json"
    source_raw_path = tmp_path / "source_raw.json"
    ...
    _write_json(
        ...
        [
            {
                "id": qid,
                "question": "Which case has the higher monetary claim: CA 004/2025 or SCT 295/2025?",
                "answer_type": "name",
            }
        ],
    )
            ...
            "records": [
                {
                    ...
                    "minimal_required_support_pages": ["doc-a_1", "doc-b_1"],
                    "notes": "Title pages identify the compared case files.",
                }
            ]
        },
    )
    ...
    record = report["records"][0]
    assert record["question_id"] == qid
    assert record["compare_kind"] == "monetary_claim_compare"
