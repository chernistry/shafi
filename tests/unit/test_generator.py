...
def test_build_prompt_strict_uses_strict_user_template(mock_settings):
    ...
    # Output rules now in system prompt for caching
    assert "Output rules:" in system
    assert "Output ONLY the extracted value" in system
...
def test_extract_citations():
    ...
    chunks = _make_chunks(3)
    answer = "The period is 3 years (cite: doc1:0:0:abc). For type 1 it is 4 years (cite: doc1:1:0:abc)."
...
def test_extract_doc_title_from_summary_supports_plain_narrative_summary() -> None:
    ...
    summary = (
        "This document is the DIFC Leasing Law, consolidated as of March 2022 and amended by DIFC Law No. 2 of 2022."
    )
...
def test_cleanup_named_amendment_answer_rebuilds_complete_amended_law_list() -> None:
    ...
    chunks = [
        RankedChunk(
            ...
            section_path="page:1",
            text=(
                "We hereby enact on this 5th day of November 2018 the DIFC Laws Amendment Law DIFC Law No. 8 of 2018."
            ),
            retrieval_score=0.97,
            rerank_score=0.97,
...
def test_build_structured_free_text_answer_handles_amended_by_enumeration_query(mock_settings) -> None:
    ...
    chunks = [
        RankedChunk(
            ...
            section_path="page:1",
            text=(
                "FOUNDATIONS LAW DIFC LAW NO. 3 OF 2018 As Amended by DIFC Laws Amendment Law DIFC Law No. 2 of 2022"
            ),
            retrieval_score=0.97,
            rerank_score=0.97,
        ...
        RankedChunk(
            ...
            doc_type=DocType.STATUTE,
            section_path="page:1",
            text=("COMPANIES LAW DIFC LAW NO. 5 OF 2018 As Amended by DIFC Laws Amendment Law DIFC Law No. 2 of 2022"),
            retrieval_score=0.96,
            rerank_score=0.96,
...
def test_build_structured_free_text_answer_handles_case_outcome_result_query(mock_settings) -> None:
    ...
    assert answer == (
        "The Defendant's Application for immediate judgment and/or strike out is dismissed (cite: cfi067:0)."
    )
...
def test_build_structured_free_text_answer_handles_specific_order_without_falling_back_to_reasons(
    mock_settings,
) -> None:
    from rag_challenge.llm.generator import RAGGenerator
...
def test_build_structured_free_text_answer_handles_set_aside_application_without_judge_name_noise(
    mock_settings,
) -> None:
    from rag_challenge.llm.generator import RAGGenerator
...
def test_cleanup_list_answer_preamble_trims_uncited_preface() -> None:
    ...
    assert (
        cleaned
        == "1. Schedule 1 Structure: All three laws state that Schedule 1 contains interpretative provisions (cite: c0, c1, c2)."
    )
...
def test_cleanup_numbered_list_items_drops_common_elements_doc_specific_items() -> None:
    ...
    assert (
        cleaned
        == "1. Interpretative provisions apply to the respective Law, and defined terms are listed (cite: c0, c1)."
    )
...
def test_cleanup_named_administration_answer_supports_difca_and_regulations_made_under_it() -> None:
    ...
    assert cleaned == ("1. Leasing Law 2020: DIFCA (cite: leasing:admin)\n2. Trust Law 2018: DIFCA (cite: trust:admin)")
...
def test_cleanup_named_administration_answer_extracts_single_law_title_from_question() -> None:
    ...
    assert (
        cleaned == "Registrar administers Foundations Law 2018 and any Regulations made under it (cite: foundations:0)"
    )
...
def test_cleanup_named_administration_answer_prefers_extracted_title_over_garbage_ref() -> None:
    ...
    chunks = [
        RankedChunk(
            ...
            doc_type=DocType.STATUTE,
            section_path="page:4",
            text=("FOUNDATIONS LAW 2018. Administration of this Law. This Law is administered by the Registrar."),
            retrieval_score=0.95,
            rerank_score=0.95,
            doc_summary="**Document Title:** Foundations Law 2018",
        ),
...
def test_build_structured_free_text_answer_handles_single_ref_fine_question(mock_settings) -> None:
    ...
    chunks = [
        RankedChunk(
            ...
            text=(
                "SCHEDULE 3 FINES AND FEES PART 1 10(7) Person failing to provide a copy of assignment of rights $1,500"
            ),
            retrieval_score=0.99,
            rerank_score=0.99,
...
def test_build_structured_free_text_answer_handles_single_doc_registrar_authority_question(mock_settings) -> None:
    ...
    assert answer == (
        "The Board of Directors of the DIFCA appoints and may dismiss the Registrar. (cite: operating:registrar)"
    )
...
async def test_generate_stream_falls_back_to_estimated_usage_when_provider_missing(mock_settings):
    ...
    llm.stream_generate_with_cascade = fake_stream_generate
    llm.get_last_stream_model = MagicMock(return_value="")
    llm.get_last_stream_usage = MagicMock(return_value={"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0})
...
# ── _select_best_citation_chunks ─────────────────────────────────────────────


def _make_ranked_chunk(chunk_id: str, text: str) -> RankedChunk:
    return RankedChunk(
        chunk_id=chunk_id,
        doc_id="doc1",
        doc_title="Test Law",
...
def test_select_best_citation_chunks_ranks_by_query_overlap() -> None:
    """High-overlap chunk wins over positional leader."""
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin

    chunks = [
        _make_ranked_chunk("c0", "general background information unrelated"),
        _make_ranked_chunk("c1", "the limitation period for this action is three years"),
    ]
    result = GenerationLogicMixin._select_best_citation_chunks(
        query="What is the limitation period for this action?",
        context_chunks=chunks,
        max_citations=1,
    )
    # c1 has higher overlap — should be picked over positional top (c0)
    assert result == ["c1"]


def test_select_best_citation_chunks_empty_input() -> None:
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin

    assert (
        GenerationLogicMixin._select_best_citation_chunks(
            query="anything",
            context_chunks=[],
            max_citations=2,
        )
...
def test_select_best_citation_chunks_respects_max_citations() -> None:
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin

    chunks = [_make_ranked_chunk(f"c{i}", f"word{i} extra") for i in range(5)]
    result = GenerationLogicMixin._select_best_citation_chunks(
        query="word1",
        context_chunks=chunks,
        max_citations=2,
...
# ── _select_scorer_citation_chunks ───────────────────────────────────────────


def _make_chunk_with_docpage(chunk_id: str, text: str = "text") -> RankedChunk:
    """chunk_id format: '{doc_id}:{0-based-page}:...' → page_id = '{doc_id}_{1-based}'"""
    return RankedChunk(
        chunk_id=chunk_id,
...
def test_select_scorer_citation_chunks_follows_scorer_order() -> None:
    """Scorer-ranked pages override position; best-scored page cited first."""
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin

    # doc1:0:... → page_id = "doc1_1", doc1:1:... → page_id = "doc1_2"
    chunks = [
        _make_chunk_with_docpage("doc1:0:a1b2c3d4"),
        _make_chunk_with_docpage("doc1:1:e5f6a7b8"),
    ]
    # Scorer says page doc1_2 is best (higher confidence)
    result = GenerationLogicMixin._select_scorer_citation_chunks(
        scorer_page_ids=["doc1_2", "doc1_1"],
        context_chunks=chunks,
        max_citations=1,
    )
    assert result == ["doc1:1:e5f6a7b8"], "Scorer's top page should be cited"


def test_select_scorer_citation_chunks_filters_to_context() -> None:
    """Pages not in context are skipped; falls through to context pages only."""
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin

    chunks = [_make_chunk_with_docpage("doc1:0:a1b2c3d4")]
    result = GenerationLogicMixin._select_scorer_citation_chunks(
        scorer_page_ids=["doc99_5", "doc1_1"],  # doc99_5 not in context
        context_chunks=chunks,
        max_citations=2,
    )
    assert result == ["doc1:0:a1b2c3d4"]


def test_select_scorer_citation_chunks_empty_scorer_returns_empty() -> None:
    """Empty scorer list → empty result (caller falls back to Jaccard)."""
    from rag_challenge.core.pipeline.generation_logic import GenerationLogicMixin
