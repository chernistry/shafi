...
from shafi.ml.grounding_dataset import (
    DocCandidateRecord,
    GroundingMlRow,
    PageCandidateRecord,
...
from shafi.ml.hard_negative_miner import HardNegativeMiner
from shafi.ml.synthetic_qa_factory import BridgeFactRecord, QuestionFamily, SyntheticQAExample
from shafi.ml.teacher_labels import (
    TeacherLabelBuilder,
    build_page_texts_from_registry,
    write_pairwise_labels_jsonl,
    write_training_triples_jsonl,
)
from shafi.models import CaseObject, CaseParty, CorpusRegistry, LawObject, LegalDocType
...
def _grounding_row() -> GroundingMlRow:
    return GroundingMlRow(
        ...
        support_fact_features=SupportFactFeatureRecord(
            doc_ref_count=1, explicit_anchor_count=1, target_page_roles_count=1
        ),
