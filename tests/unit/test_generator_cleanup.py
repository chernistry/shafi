...
def test_truncated_tail_valid_enumerated_item_ending_with_of() -> None:
    assert looks_like_truncated_tail("Application of Civil and Commercial Laws in the DIFC") is False


def test_truncated_tail_valid_text_ending_with_and() -> None:
    assert looks_like_truncated_tail("Partners are jointly and severally liable") is False


def test_truncated_tail_valid_text_ending_with_the() -> None:
    assert looks_like_truncated_tail("This law is administered by the Registrar of the") is False
