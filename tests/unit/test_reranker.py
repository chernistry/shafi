...
def mock_settings():
    ...
    with (
        patch("shafi.core.reranker.get_settings", return_value=settings),
        patch("shafi.core.request_limiter._LIMITER_REGISTRY", {}),
    ):
        yield settings
    ...
    cohere_client = SimpleNamespace(
        rerank=AsyncMock(
            return_value=SimpleNamespace(
                results=[
                    _cohere_result(0, 0.1),
                    _cohere_result(1, 0.8),
                    _cohere_result(2, 0.5),
                    _cohere_result(3, 0.2),
                    _cohere_result(4, 0.4),
                ]
            )
        )
    )
...
async def test_circuit_breaker_opens_after_three_failures_and_skips_zerank(mock_settings):
    from shafi.core.reranker import CircuitState, RerankerClient
...
async def test_prefer_fast_uses_cohere_first(mock_settings):
    ...
    cohere_client = SimpleNamespace(
        rerank=AsyncMock(
            return_value=SimpleNamespace(
                results=[_cohere_result(0, 0.9), _cohere_result(1, 0.8), _cohere_result(2, 0.7)]
            )
        )
    )
    async with httpx.AsyncClient(transport=transport) as client:
