...
def test_ocr_fallback_synthetic_pack_reflects_page_preserving_fallback(monkeypatch) -> None:
    ...
    for case in cases:
        ...
        monkeypatch.setattr(
            parser, "_parse_pdf_pymupdf_pages", lambda _path, pymupdf_pages=pymupdf_pages: pymupdf_pages
        )
        monkeypatch.setattr(
            parser, "_is_pdf_text_sufficient", lambda _text, text_sufficient=text_sufficient: text_sufficient
        )
        monkeypatch.setattr(
            parser,
