...
def test_import_reviewed_labels_derives_confidence_slices(tmp_path: Path) -> None:
    ...
    high_benchmark = json.loads((output_dir / "reviewed_page_benchmark_high_confidence_81.json").read_text(encoding="utf-8"))
    assert high_benchmark["cases"] == [
        {"question_id": "q-high", "gold_page_ids": ["doc_1"], "trust_tier": "trusted"}
    ]
...
def test_import_reviewed_labels_rejects_id_mismatch(tmp_path: Path) -> None:
    ...
    reviewed_golden.write_text(json.dumps([_build_reviewed_row("q1", confidence="high", pages=["doc_1"])]), encoding="utf-8")
    reviewed_page_benchmark.write_text(
