...
def test_search_bounded_support_subsets_picks_gold_page_subset(tmp_path: Path) -> None:
    ...
    baseline_submission.write_text(
        json.dumps({"architecture_summary": {}, "answers": [_submission_record(qid=qid, answer=False, page_id="doc_bad_1")]}),
        encoding="utf-8",
    )
    ...
    page_submission.write_text(
        json.dumps({"architecture_summary": {}, "answers": [_submission_record(qid=qid, answer=False, page_id="doc_good_2")]}),
        encoding="utf-8",
    )
    page_raw.write_text(
        json.dumps([_raw_record(qid=qid, answer_text="False", used_page_id="doc_good_2", context_page_id="doc_good_2")]),
        encoding="utf-8",
    )
