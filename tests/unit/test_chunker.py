...
from shafi.models import Chunk, DocType, DocumentSection, ParsedDocument, ProvidedChunk
...
def mock_chunker_settings():
    ...
    with patch("shafi.ingestion.chunker.get_settings", return_value=settings):
        yield settings
...
def mock_sac_settings():
    ...
    with patch("shafi.ingestion.sac.get_settings", return_value=settings):
        yield settings
