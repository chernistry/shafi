        ...
        json.dumps(
            {
                ...
                "source_eval": "seed",
                "description": "",
                "cases": [{"question_id": qid} for qid in qids],
            },
            indent=2,
        ),
        encoding="utf-8",
...
def test_build_blindspots_surfaces_missing_anchor_case_not_in_benchmark(tmp_path: Path) -> None:
        ...
        [
            {
                ...
                "minimal_required_support_pages": ["golddoc_1"],
            }
        ],
    )
    ...
    _write_raw_results(
        ...
        [
            {
                "case": {"case_id": "qid-1"},
                "telemetry": {"used_page_ids": ["otherdoc_4"], "context_page_ids": [], "retrieved_page_ids": []},
            }
        ],
    )
    ...
    assert blindspots[0].route_family == "explicit_page_anchor"
    assert blindspots[0].family_tags == ["title_page", "explicit_page"]
...
def test_build_blindspots_keeps_benchmark_presence_flag(tmp_path: Path) -> None:
    ...
    _write_raw_results(
        ...
        [
            {
                "case": {"case_id": "qid-1"},
                "telemetry": {"used_page_ids": ["doc_2"], "context_page_ids": [], "retrieved_page_ids": []},
            }
        ],
    )
    ...
    assert blindspots[0].route_family == "explicit_page_anchor"
    assert blindspots[0].family_tags == ["page_2", "explicit_page"]


def test_build_blindspots_marks_multi_doc_and_same_doc_family_tags(tmp_path: Path) -> None:
    scaffold = tmp_path / "scaffold.json"
    benchmark = tmp_path / "benchmark.json"
    raw_results = tmp_path / "raw_results.json"
    _write_scaffold(
        ...
        [
            {
                "question_id": "qid-1",
                "question": "Across all documents in both cases, is there any common party?",
                "manual_verdict": "correct",
                "failure_class": "support_undercoverage",
                "minimal_required_support_pages": ["doc_1_1", "doc_2_1"],
            }
        ],
    )
    _write_benchmark(benchmark, [])
    _write_raw_results(
        ...
        [
            {
                "case": {"case_id": "qid-1"},
                "telemetry": {"used_page_ids": ["doc_1_3"], "context_page_ids": [], "retrieved_page_ids": []},
            }
        ],
    )
    ...
    assert len(blindspots) == 1
    assert blindspots[0].route_family == "multi_doc_comparison"
    assert "multi_doc_comparison" in blindspots[0].family_tags
    assert "same_doc_miss" in blindspots[0].family_tags
