...
from rag_challenge.core.grounding.evidence_selector import (
...
class TestShouldActivateSidecar:
    ...
    def test_compare_pair_always_active(self) -> None:
        ...
        assert GroundingEvidenceSelector._should_activate_sidecar(
            scope=scope, answer_type="boolean", context_chunks=chunks,
        ) is True
    ...
    def test_full_case_files_always_active(self) -> None:
        ...
        chunks = [_make_chunk("docA", 0)]
        assert GroundingEvidenceSelector._should_activate_sidecar(
            scope=scope, answer_type="text", context_chunks=chunks,
        ) is True
    ...
    def test_single_field_inactive(self) -> None:
        scope = _make_scope(mode=ScopeMode.SINGLE_FIELD_SINGLE_DOC)
        chunks = [_make_chunk("docA", 0)]
        assert GroundingEvidenceSelector._should_activate_sidecar(
            scope=scope, answer_type="number", context_chunks=chunks,
        ) is False

    def test_broad_free_text_inactive(self) -> None:
        scope = _make_scope(mode=ScopeMode.BROAD_FREE_TEXT)
        chunks = [_make_chunk("docA", 0)]
        assert GroundingEvidenceSelector._should_activate_sidecar(
            scope=scope, answer_type="free_text", context_chunks=chunks,
        ) is False
...
class TestSelectMinimalPages:
    ...
    def test_empty_scored_returns_empty(self) -> None:
        sel = _make_selector()
        scope = _make_scope()
        assert sel._select_minimal_pages(scope=scope, scored={}) == []
    ...
    def test_explicit_page_returns_top_1(self) -> None:
        ...
        result = sel._select_minimal_pages(scope=scope, scored=scored)
        assert result == ["docA_1"]
    ...
    def test_default_mode_respects_page_budget(self) -> None:
        sel = _make_selector()
