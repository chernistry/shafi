...
def test_lookup_field_prioritizes_case_number_over_generic_party_noise() -> None:
    ...
    assert all(
        entity.mention_text.casefold() not in {"who", "were", "the", "case"} for entity in contract.primary_entities
    )
    assert all(
        entity.mention_text.casefold() not in {"who", "were", "the", "case"} for entity in contract.constraint_entities
    )
