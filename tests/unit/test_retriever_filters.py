...
from rag_challenge.core.retriever_filters import expand_doc_ref_variants
...
def test_build_filter_must_not_doc_ids_excludes_documents() -> None:
    from rag_challenge.core.retriever_filters import build_filter
...
def test_build_filter_case_ref_includes_doc_title_match_text() -> None:
    ...
    should_filter = next(
        f for f in result.must if isinstance(f, models.Filter)  # type: ignore[union-attr]
    )
    ...
    match_text_conditions = [
        ...
        if hasattr(c, "key") and c.key == "doc_title"  # type: ignore[union-attr]
        and hasattr(c, "match") and isinstance(c.match, models.MatchText)  # type: ignore[union-attr]
        and c.match.text == "CFI 057/2025"  # type: ignore[union-attr]
    ]
    assert len(match_text_conditions) == 1, (
        "Expected exactly one MatchText condition on doc_title for CFI 057/2025"
    )
...
def test_build_filter_case_ref_canonical_only_match_text() -> None:
    ...
    assert "SCT 415" not in match_texts, "Short form without year must NOT be present — causes year-collision false-positives"
...
def test_doc_title_filter_variants_includes_uppercase_for_plain_titles() -> None:
    ...
    """
    from rag_challenge.core.retriever_filters import doc_title_filter_variants
