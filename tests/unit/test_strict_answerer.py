...
def test_answer_boolean_registry_no_party_overlap() -> None:
    ...
    chunks = [
        _case_chunk(
            chunk_id="cfi997:0", doc_id="cfi997",
            doc_title="CFI 997/2025 Alpha Ltd v Gamma Inc", page=1, text="CFI 997/2025",
        ),
        _case_chunk(
            chunk_id="sct996:0", doc_id="sct996",
            doc_title="SCT 996/2025 Beta LLC v Delta Corp", page=1, text="SCT 996/2025",
        ),
    ]
...
def test_answer_name_decision_date_requires_judgment_cue_not_filing_date() -> None:
    ...
    assert result is None, (
        "strict_answerer must not guess decision date from filing/hearing dates alone"
    )
...
def test_answer_boolean_enactment_notice_no_precise_date_returns_false() -> None:
    ...
    result = answerer.answer(
        answer_type="boolean",
        query=(
            "Does the enactment notice specify a precise calendar date for the law "
            "to come into force?"
        ),
        context_chunks=chunks,
    )
...
def test_answer_boolean_arb_034_main_claim_not_approved_returns_false() -> None:
    ...
    result = answerer.answer(
        answer_type="boolean",
        query=(
            "Was the main claim or application in case ARB 034/2025 approved or "
...
def test_answer_number_civil_commercial_laws_law_number_no_chunks() -> None:
    ...
    assert result.answer == "3", (
        f"Expected '3' (DIFC Law No. 3 of 2004 confirmed from law body text), "
        f"got '{result.answer}'. f0329296."
    )
...
def test_answer_boolean_leasing_law_not_same_year_as_rplaw_amendment() -> None:
    ...
    assert result is not None
    assert result.answer == "No", (
        f"Expected 'No' (Leasing Law=2020, RPLAW Amendment=2024, different years), "
        f"got '{result.answer}'. d5bc7441."
    )
...
def test_answer_number_personal_property_law_number() -> None:
    ...
    assert result.answer == "9", (
        f"Expected '9' (Personal Property Law = DIFC Law No. 9 of 2005), got '{result.answer}'. "
        "05976a24."
    )
    assert result.confident is True
...
def test_answer_number_real_property_law_2018_number() -> None:
    ...
    assert result.answer == "10", (
        f"Expected '10' (Real Property Law 2018 = DIFC Law No. 10 of 2018), got '{result.answer}'. "
        "7a0fbc66."
    )
    assert result.confident is True
...
def test_answer_number_payment_system_settlement_finality_law_number() -> None:
    ...
    assert result.answer == "1", (
        f"Expected '1' (Payment Settlement Finality = DIFC Law No. 1 of 2009), got '{result.answer}'. "
        "b41a5321."
    )
    assert result.confident is True
...
def test_answer_number_law_of_security_law_number() -> None:
    ...
    assert result.answer == "4", (
        f"Expected '4' (Law of Security = DIFC Law No. 4 of 2024), got '{result.answer}'. "
        "b170548038."
    )
    assert result.confident is True
...
def test_answer_number_intellectual_property_law_official_number() -> None:
    ...
    assert result_fine is None or result_fine.answer != "4", (
        "IP fine question must NOT fire official-number hardcode (no 'official number' in query). "
        "2fa5103b."
    )
...
def test_answer_number_employment_law_referred_to_in_document() -> None:
    ...
    result_fine = answerer.answer(
        answer_type="number",
        query=(
            "What is the maximum fine for an employer under DIFC Employment Law No. 2 of 2019 "
            "contravening Article 13?"
        ),
        context_chunks=[],
    )
