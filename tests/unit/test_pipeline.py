...
from shafi.core.query_contract import ExecutionEngine
from shafi.core.verifier import VerificationResult
...
from shafi.models.legal_objects import CaseObject, CaseParty, CorpusRegistry, LawObject, LegalDocType
    ...
    registry = CorpusRegistry(
        ...
        cases={
            "cfi_001_2024": CaseObject(
                ...
                parties=[CaseParty(name="Alpha Ltd", role="Claimant"), CaseParty(name="Beta Ltd", role="Respondent")],
            ),
            "cfi_002_2024": CaseObject(
                ...
                parties=[CaseParty(name="Gamma Ltd", role="Claimant"), CaseParty(name="Beta Ltd", role="Respondent")],
            ),
        },
    )
...
def test_extract_question_title_refs_strips_generic_question_leads() -> None:
    from shafi.core.pipeline import _extract_question_title_refs
...
async def test_pipeline_runs_end_to_end(pipeline_builder):
    ...
    assert result["query_contract"].execution_plan == [ExecutionEngine.STANDARD_RAG]
...
@pytest.mark.asyncio
async def test_pipeline_invokes_selective_icr_shadow_rerank_without_changing_main_ranking(
    pipeline_builder,
    mock_settings,
):
    shadow_ranked = _make_ranked(4)
    mock_settings.reranker.shadow_selective_icr_enabled = True
    pipeline_builder._reranker.rerank = AsyncMock(return_value=_make_ranked(6))
    pipeline_builder._reranker.shadow_rerank = AsyncMock(return_value=shadow_ranked)
    pipeline_builder._reranker.get_last_used_model = MagicMock(return_value="zerank-2")
    pipeline_builder._reranker.get_last_shadow_used_model = MagicMock(return_value="selective_icr_heuristic")
    pipeline_builder._reranker.get_last_shadow_latency_ms = MagicMock(return_value=17)
    pipeline_builder._reranker.get_last_shadow_candidate_count = MagicMock(return_value=10)
    pipeline_builder._reranker.get_last_shadow_chunk_ids = MagicMock(
        return_value=[chunk.chunk_id for chunk in shadow_ranked]
    )
    pipeline_builder._reranker.get_last_shadow_page_ids = MagicMock(return_value=[])

    app = pipeline_builder.compile()
    collector = TelemetryCollector(request_id="shadow-test")

    result = await app.ainvoke(
        {
            "query": "What is the limitation period?",
            "request_id": "shadow-test",
            "collector": collector,
        }
    )

    telemetry = result["telemetry"]
    assert pipeline_builder._reranker.rerank.await_count == 1
    assert pipeline_builder._reranker.shadow_rerank.await_count == 1
    assert telemetry.model_rerank == "zerank-2"
    assert telemetry.selective_icr_shadow_used is True
    assert telemetry.selective_icr_shadow_model == "selective_icr_heuristic"
    assert telemetry.selective_icr_shadow_latency_ms == 17
    assert telemetry.selective_icr_shadow_candidate_count == 10
    assert telemetry.selective_icr_shadow_chunk_ids == [chunk.chunk_id for chunk in shadow_ranked]
    assert telemetry.selective_icr_shadow_page_ids == []


@pytest.mark.asyncio
async def test_decompose_skips_structured_contract_queries(mock_settings, tmp_path) -> None:
    from shafi.core.pipeline import RAGPipelineBuilder

    mock_settings.pipeline.enable_multi_hop = True
    ...
    classified = await builder._classify(
        {
            "query": "Compare CFI 001/2024 and CFI 002/2024 under the Companies Law.",
            "request_id": "contract-test",
            "collector": TelemetryCollector(request_id="contract-test"),
        }
    )
    compiled = await builder._compile_query_contract(
        {
            **classified,
            "request_id": "contract-test",
            "collector": TelemetryCollector(request_id="contract-test"),
    ...
    decomposed = await builder._decompose(
        {
            **classified,
            **compiled,
            "request_id": "contract-test",
    ...
    assert compiled["query_contract"].execution_plan == [ExecutionEngine.COMPARE_JOIN]
...
async def test_retrieve_single_title_strict_query_uses_targeted_title_lookup(pipeline_builder) -> None:
    ...
    # Three retrieve calls: 1) parallel sparse-first (no doc_refs), 2) hybrid fallback
    # (sparse returned < threshold for date type), 3) targeted title retrieval.
    pipeline_builder._retriever.retrieve = AsyncMock(
        side_effect=[generic_retrieved, generic_retrieved, targeted_retrieved],
    )
...
async def test_pipeline_strict_boolean_normalizes_invalid_citations(mock_settings):
    ...
    async def _gen_stream_bool(*args, **kwargs):
        yield "Yes"

    generator.generate_stream = _gen_stream_bool
    generator.extract_cited_chunk_ids = MagicMock(
        side_effect=lambda text: (
            ["bad-id (cite: c0"] if "bad-id" in str(text) else (["c0"] if "c0" in str(text) else [])
        )
    )
    generator.extract_citations = MagicMock(return_value=[Citation(chunk_id="c0", doc_title="Test")])
    generator.sanitize_citations = MagicMock(side_effect=lambda answer, _ctx: answer)
...
async def test_pipeline_strict_number_localizes_minimal_support_instead_of_whole_context(mock_settings):
    ...
    async def _gen_stream_number(*args, **kwargs):
        yield "250499.26"

    generator.generate_stream = _gen_stream_number
...
async def test_pipeline_strict_number_without_support_does_not_fallback_to_first_context_chunk(mock_settings):
    ...
    async def _gen_stream_number2(*args, **kwargs):
        yield "250499.26"

    generator.generate_stream = _gen_stream_number2
    generator.extract_cited_chunk_ids = MagicMock(return_value=[])
    generator.extract_citations = MagicMock(return_value=[])
    generator.sanitize_citations = MagicMock(side_effect=lambda answer, _ctx: answer)
    ...
    assert result["answer"] == "250499.26"
    # F-beta 2.5 recall optimization: when localization fails, cite the 2
    # context chunks with highest query-text Jaccard overlap.
    cited = collector.finalize().cited_chunk_ids
    assert len(cited) == 2
    assert set(cited) == {"noise:0", "noise:1"}
...
async def test_strict_null_preserves_context_page_telemetry(mock_settings):
    from shafi.core.pipeline import RAGPipelineBuilder
    from shafi.core.strict_answerer import StrictAnswerResult
    ...
    builder._strict_answerer.answer = MagicMock(
        return_value=StrictAnswerResult(answer="null", cited_chunk_ids=[], confident=True)
    )
...
async def test_pipeline_free_text_used_pages_can_extend_beyond_inline_citations(mock_settings):
    ...
    generator.extract_citations = MagicMock(
        return_value=[Citation(chunk_id="data:0:0:title", doc_title="Data Protection Law 2020")]
    )
    generator.sanitize_citations = MagicMock(side_effect=lambda answer, _ctx: answer)
    generator.cleanup_final_answer = MagicMock(side_effect=lambda answer: answer)
...
async def test_generate_adds_citation_title_hint_for_registrar_enumeration(mock_settings):
    ...
    generator.extract_citations = MagicMock(
        return_value=[Citation(chunk_id="c0", doc_title="Limited Partnership Law 2006")]
    )
...
async def test_generate_adds_structural_common_elements_hint_and_strips_trailing_negative(mock_settings):
    ...
    assert (
        result["answer"]
        == "Schedule 1 in both laws contains interpretative provisions and a list of defined terms (cite: c0, c1)."
    )
...
async def test_generate_named_title_refs_query_runs_named_ref_cleanup(mock_settings):
    ...
    generator.cleanup_named_ref_enumeration_items = MagicMock(
        side_effect=lambda answer, **_kwargs: "1. Investment Companies (IC) Regulations (cite: c1)"
    )
    generator.cleanup_list_answer_postamble = MagicMock(side_effect=lambda answer: answer)
    generator.strip_negative_subclaims = MagicMock(side_effect=lambda answer: answer)
...
async def test_generate_ruler_enactment_query_runs_cleanup(mock_settings):
    ...
    generator.extract_citations = MagicMock(
        return_value=[Citation(chunk_id="c1", doc_title="Law of Damages and Remedies 2005")]
    )
...
async def test_generate_answer_final_uses_account_effective_dates_cleanup(mock_settings):
    ...
    generator.generate = AsyncMock()
    generator.extract_citations = MagicMock(
        return_value=[Citation(chunk_id="c0", doc_title="Common Reporting Standard Law 2018")]
    )
    generator.extract_cited_chunk_ids = MagicMock(return_value=["c0", "c1"])
    generator.sanitize_citations = MagicMock(side_effect=lambda answer, _ctx: answer)
    generator.cleanup_list_answer_preamble = MagicMock(side_effect=lambda answer: answer)
...
async def test_generate_named_administration_query_uses_structured_bypass_and_sets_debug_telemetry(mock_settings):
    ...
    generator.extract_citations = MagicMock(
        return_value=[Citation(chunk_id="c0", doc_title="Limited Liability Partnership Law 2004")]
    )
    generator.extract_cited_chunk_ids = MagicMock(return_value=["c0", "c1"])
    generator.sanitize_citations = MagicMock(side_effect=lambda answer, _ctx: answer)
    generator.cleanup_final_answer = MagicMock(side_effect=lambda answer: answer)
...
@pytest.mark.asyncio
async def test_rerank_passes_instruction_conditioning_for_article_query(mock_settings):
    from shafi.core.pipeline import RAGPipelineBuilder
    ...
    builder._reranker.rerank = AsyncMock(return_value=_make_ranked(3))
    collector = TelemetryCollector(request_id="instruction-family")

    await builder._rerank(
        {
            "query": "According to Article 16 of the Operating Law 2018, what document must every Registered Person file?",
            "collector": collector,
            "answer_type": "name",
    ...
    kwargs = builder._reranker.rerank.await_args.kwargs
    assert kwargs["instruction_family"] == "exact_provision"
    assert "exact article" in kwargs["instruction"]
...
async def test_retrieve_caps_hybrid_candidates_for_strict_queries(mock_settings):
    ...
    retriever = MagicMock()
    retriever.embed_query = AsyncMock(return_value=[0.1] * 8)
    # First call: parallel sparse-first returns too few for "name" threshold (< 3).
    # Second call: hybrid fallback fires with capped prefetch values.
    # Third call (title retrieval): returns empty.
    sparse_few = _make_retrieved(2)
    hybrid_result = _make_retrieved(10)
    retriever.retrieve = AsyncMock(side_effect=[sparse_few, hybrid_result, []])
    ...
    # embed_query may run as a background prefetch task -- verify at least one call
    assert retriever.embed_query.call_count >= 1
    # The hybrid fallback (second retrieve call) should use capped strict overrides
    hybrid_call = retriever.retrieve.call_args_list[1]
    assert hybrid_call.kwargs["top_k"] == 20
    assert hybrid_call.kwargs["prefetch_dense"] == 24
    assert hybrid_call.kwargs["prefetch_sparse"] == 24
...
async def test_retrieve_caps_title_multi_retrieve_for_common_elements_queries(mock_settings):
    ...
    # Filter to title retrieval calls: sparse_only=True with an explicit top_k cap.
    # The initial parallel sparse-first call (top_k=None) is excluded.
    title_calls = [
        call
        for call in retriever.retrieve.await_args_list
        if call.kwargs.get("sparse_only") is True and call.kwargs.get("top_k") is not None
    ]
    assert title_calls
...
async def test_retrieve_targets_article_anchored_strict_doc_ref_clause(mock_settings):
    ...
    retriever = MagicMock()
    # Call 1: sparse filtered to doc_ref → 1 chunk (below sparse_fallback_threshold=3 for number type)
    # Call 2: dense fallback with doc_ref filter (triggered by threshold < 3)
    # Call 3: article-anchor targeted sparse (adds specific article chunk)
    retriever.retrieve = AsyncMock(
        side_effect=[
            [
                _make_retrieved_chunk(
            ...
            # Dense fallback returns broader coverage of the document
            [
                _make_retrieved_chunk(
                    chunk_id="employment:title",
                    doc_id="employment",
                ...
                _make_retrieved_chunk(
                    chunk_id="employment:scope",
                    doc_id="employment",
                    doc_title="EMPLOYMENT LAW",
                    section_path="page:5",
                    text="This Law applies to all Employers and Employees working in the DIFC.",
                    score=0.81,
                    doc_summary="Employment Law 2019, DIFC Law No. 2 of 2019.",
                ),
                _make_retrieved_chunk(
    ...
    retriever.embed_query = AsyncMock(return_value=[0.1] * 1792)
    ...
    # 3 calls: sparse(doc_ref) → dense fallback (threshold<3 for number type) → article-anchor sparse
...
def test_ensure_named_commencement_context_single_ref_boosts_correct_doc(mock_settings):
    from shafi.core.pipeline import RAGPipelineBuilder

    del mock_settings
    # Single-ref commencement query: "When did the LEASING REGULATIONS come into force?"
    # The correct chunk (FCR cover page) is ranked low by default reranker but should be boosted.
    reranked = [
        RankedChunk(
            chunk_id="other:title",
            doc_id="other",
            doc_title="NETTING LAW",
            doc_type=DocType.STATUTE,
            section_path="page:3",
            text='This Law may be cited as the "Netting Law 2014". Date of enactment and commencement.',
            retrieval_score=0.92,
            rerank_score=0.92,
            doc_summary="",
        ),
        RankedChunk(
            chunk_id="leasing:cover",
            doc_id="leasing",
            doc_title="LEASING REGULATIONS",
            doc_type=DocType.STATUTE,
            section_path="page:1",
            text="LEASING REGULATIONS In force on 15 March 2018",
    ...
    selected = RAGPipelineBuilder._ensure_named_commencement_context(
        query="When did the LEASING REGULATIONS come into force?",
    ...
    # The leasing cover chunk should be boosted to first position
    assert selected[0].chunk_id == "leasing:cover"
...
def test_boolean_admin_compare_context_recovers_canonical_chunk_outside_capped_pool(mock_settings):
    ...
    retrieved = [
        ...
        _make_retrieved_chunk(
            ...
            section_path="page:4",
            text=(
                "TRUST LAW DIFC Law No. 4 of 2018. Administration of this Law. This Law is administered by the DIFCA."
            ),
            score=0.98,
...
def test_is_remuneration_recordkeeping_query_detects_article_16_recordkeeping_phrase() -> None:
    from shafi.core.pipeline import _is_remuneration_recordkeeping_query
...
def test_remuneration_recordkeeping_clause_score_prefers_pay_period_clause(pipeline_builder) -> None:
    ...
    assert pipeline_builder._remuneration_recordkeeping_clause_score(
        best
...
def test_select_targeted_title_seed_chunk_id_prefers_title_year_page_for_same_year_boolean(pipeline_builder) -> None:
    row = [
        _make_retrieved_chunk(
            ...
            text=("The Employment Law 2019 repeals and replaces the Employment Law 2005 (DIFC Law No. 4 of 2005)."),
            score=0.97,
        ),
        _make_retrieved_chunk(
            chunk_id="employment:title",
...
def test_ensure_boolean_admin_compare_context_prefers_one_admin_page_per_ref(mock_settings):
    ...
    retrieved = [
        _make_retrieved_chunk(
            ...
            text=("Cross-reference note mentioning the Trust Law No. 4 of 2018. This is not an administration clause."),
            score=0.99,
        ),
        _make_retrieved_chunk(
            chunk_id="ip:admin",
        ...
        _make_retrieved_chunk(
            ...
            section_path="page:5",
            text=("Administration of this Law. The DIFC Authority shall administer this Law."),
            score=0.51,
        ),
        _make_retrieved_chunk(
            chunk_id="trust:title",
...
def test_rerank_support_pages_within_selected_docs_picks_one_page_per_doc_for_compare(mock_settings):
    ...
    # With per_doc_pages=2 for compare queries (F-beta 2.5 recall priority),
    # both title pages plus body pages are returned.
    assert "ca4:title" in reranked_ids
    assert "arb34:title" in reranked_ids
    assert len(reranked_ids) >= 2
...
def test_rerank_support_pages_within_selected_docs_caps_single_doc_strict_to_one_page(mock_settings):
    ...
    # With F-beta 2.5, strict_single_doc returns top-2 pages for recall.
    assert reranked_ids == ["law:page5", "law:page3"]
...
def test_apply_doc_shortlist_gating_combines_doc_refs_with_title_refs_for_admin_compare(mock_settings):
    ...
    retrieved = [
        ...
        _make_retrieved_chunk(
            ...
            section_path="page:4",
            text=(
                "This Law may be cited as the Trust Law No. 4 of 2018. The DIFC Authority shall administer this Law."
            ),
            score=0.83,
        ),
    ]
...
def test_multi_criteria_enumeration_detects_comma_separated_filters() -> None:
    from shafi.core.pipeline import _is_multi_criteria_enumeration_query
