...
def test_current_parser_mode_marks_multi_page_fallback_as_collapsed() -> None:
    assert _current_parser_mode(actual_pdf_pages=4, text_sufficient=False) == "docling_merged_single_page"
    assert _current_parser_mode(actual_pdf_pages=4, text_sufficient=True) == "pymupdf_pages_preserved"
    assert _current_parser_mode(actual_pdf_pages=1, text_sufficient=False) == "not_applicable_single_page"
...
def test_audit_document_detects_multi_page_collapse(tmp_path: Path, monkeypatch) -> None:
    ...
    assert audit.page_identity_collapsed is True
    assert audit.current_parser_mode == "docling_merged_single_page"
