...
from shafi.submission.generate import (
    SubmissionCase,
    _coerce_answer_type,
...
def test_normalize_free_text_answer_strips_citations_and_caps_length() -> None:
    answer = "1. Foundations Law 2018 (cite: abc:0:0:1)\n2. Trust Law 2018 (cite: def:1:0:2)\n3. " + (
        "Very long explanation " * 30
    )
...
def test_normalize_free_text_answer_caps_to_three_sentences() -> None:
    answer = "Alpha is supported. Beta is supported too. Gamma remains supported. Delta should not survive projection."
...
def test_normalize_free_text_fills_budget_with_truncated_tail() -> None:
    """When sentences are dropped to fit 280 chars, the remaining budget
    should be filled with a truncated version of the next sentence."""
    # Sentence 1 is ~100 chars, sentence 2 is ~120 chars, sentence 3 is ~90 chars.
    # Total ~310 > 280. After dropping sentence 3, candidate is ~220 chars.
    # Remaining budget ~60 chars should be filled with truncated sentence 3.
    s1 = "The Trust Law DIFC Law No. 11 of 2005 requires all trustees to maintain proper accounting records."
    s2 = "These records must include details of all trust property and liabilities as specified in Article 38."
    s3 = "Failure to comply may result in removal of the trustee by the DIFC Court under applicable law."
    answer = f"{s1} {s2} {s3}"

    normalized = _normalize_free_text_answer(answer)

    assert len(normalized) <= 280
    # Should contain content from sentence 3 (truncated with "...")
    assert s1 in normalized
    assert s2 in normalized
    # The truncated tail should be present (more content than just s1+s2)
    assert len(normalized) > len(f"{s1} {s2}")


def test_normalize_free_text_truncated_tail_respects_280_limit() -> None:
    """Truncated tails must never push the answer over 280 chars."""
    s1 = "A" * 200 + "."
    s2 = "B" * 200 + "."
    answer = f"{s1} {s2}"
...
def test_normalize_free_text_keeps_year_ending_sentence() -> None:
    answer = (
...
def test_normalize_free_text_answer_keeps_law_no_clause() -> None:
    ...
    assert (
        normalized == "Registrar administers Foundations Law, DIFC Law No. 3 of 2018 and any Regulations made under it"
    )
...
# --- Format compliance: _looks_like_name allows periods in entity names ---


def test_format_compliance_names_with_periods() -> None:
    from shafi.eval.metrics import AnswerTypeFormatCompliance

    # Periods in abbreviations must pass for both "name" and "names" types
    assert AnswerTypeFormatCompliance.is_answer_format_compliant("Architeriors Interior Design (L.L.C)", "names")
    assert AnswerTypeFormatCompliance.is_answer_format_compliant("Dr. Smith", "name")
    assert AnswerTypeFormatCompliance.is_answer_format_compliant("L.L.C", "name")
    assert AnswerTypeFormatCompliance.is_answer_format_compliant("Art. 5 of DIFC Law No. 3", "name")
    # Exclamation/question marks still rejected
    assert not AnswerTypeFormatCompliance.is_answer_format_compliant("What is this?", "name")
    assert not AnswerTypeFormatCompliance.is_answer_format_compliant("Stop!", "name")
    # >12 words still rejected
    long_name = " ".join(f"word{i}" for i in range(13))
    assert not AnswerTypeFormatCompliance.is_answer_format_compliant(long_name, "name")
