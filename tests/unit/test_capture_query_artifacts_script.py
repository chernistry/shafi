...
from rag_challenge.submission.platform import PlatformCaseResult
from rag_challenge.submission.query_capture import capture_query_artifacts, summarize_results
...
def test_capture_query_artifacts_writes_outputs(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("rag_challenge.submission.query_capture._run_questions", _fake_run_questions)
    monkeypatch.setattr("rag_challenge.submission.query_capture._repair_anomalous_results", _fake_repair)
    monkeypatch.setattr(
        "rag_challenge.submission.query_capture.build_truth_audit_scaffold",
        lambda **kwargs: {"summary": {"questions_count": 1}, "records": []},
    )
    monkeypatch.setattr(
        "rag_challenge.submission.query_capture.render_truth_audit_workbook",
