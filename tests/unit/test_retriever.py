...
def mock_settings():
    ...
    with (
        patch("shafi.core.retriever.get_settings", return_value=settings),
        patch("shafi.core.retriever.BM25SparseEncoder", return_value=sparse_encoder),
    ):
        yield settings
...
async def test_dense_only_when_cloud_inference_disabled(mock_embedder, mock_store):
    ...
    with patch("shafi.core.retriever.get_settings", return_value=settings):
        from shafi.core.retriever import HybridRetriever
