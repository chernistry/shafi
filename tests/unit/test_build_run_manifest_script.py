...
def test_build_run_manifest_script_writes_deterministic_fingerprint(tmp_path: Path) -> None:
    ...
    parser_file = repo_root / "src" / "shafi" / "ingestion" / "parser.py"
    pipeline_file = repo_root / "src" / "shafi" / "core" / "pipeline.py"
    touched_file = repo_root / "src" / "shafi" / "core" / "retriever.py"
    ...
    assert manifest["touched_file_hashes"] == [
        {
            "path": "src/shafi/core/retriever.py",
            "exists": True,
            "sha256": manifest["touched_file_hashes"][0]["sha256"],
            "size_bytes": touched_file.stat().st_size,
        }
    ]
    assert manifest["core_pipeline_hashes"][0]["path"] == "src/shafi/core/pipeline.py"
