...
def test_single_field_uses_configured_budget_3() -> None:
    ...
    result = classify_query_scope(
        "Who is the claimant?", "name", settings=_make_settings(3)
    )
...
def test_single_field_uses_configured_budget_2() -> None:
    ...
    result = classify_query_scope(
        "What is the case number?", "number", settings=_make_settings(2)
    )
...
def test_anchor_query_uses_configured_budget() -> None:
    ...
    result = classify_query_scope(
        "What does Article 5 say about overtime?", "free_text", settings=_make_settings(3)
    )
...
def test_costs_query_uses_configured_budget() -> None:
    ...
    result = classify_query_scope(
        "What costs were awarded?", "number", settings=_make_settings(3)
    )
    assert result.page_budget == 3
...
def test_date_of_issue_query_uses_configured_budget() -> None:
    ...
    result = classify_query_scope(
        "What is the date of issue?", "date", settings=_make_settings(3)
    )
    assert result.page_budget == 3
...
def test_explicit_page_always_budget_1() -> None:
    ...
    result = classify_query_scope(
        "What is on page 5?", "free_text", settings=_make_settings(3)
    )
...
def test_full_case_always_budget_4() -> None:
    ...
    result = classify_query_scope(
        "Look through all documents for the outcome", "free_text", settings=_make_settings(3)
    )
...
def test_free_text_broad_always_budget_4() -> None:
    ...
    result = classify_query_scope(
        "Summarize the rights of employees", "free_text", settings=_make_settings(3)
    )
    assert result.page_budget == 4
