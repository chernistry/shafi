...
def test_build_counterfactual_candidate_uses_baseline_answers_and_candidate_pages(tmp_path: Path) -> None:
    ...
    answer_submission.write_text(
        json.dumps(
            {
                "architecture_summary": {"mode": "baseline"},
                "answers": [_submission_record(qid="q1", answer="A", page_id="doca_1")],
            }
        ),
        encoding="utf-8",
    )
    answer_raw.write_text(
        json.dumps([_raw_record(qid="q1", answer_text="A", used_page_id="doca_1", context_page_id="doca_1")]),
        encoding="utf-8",
    )
    ...
    page_submission.write_text(
        json.dumps(
            {
                "architecture_summary": {"mode": "candidate"},
    ...
    page_raw.write_text(
        json.dumps([_raw_record(qid="q1", answer_text="B", used_page_id="docb_2", context_page_id="docb_2")]),
        encoding="utf-8",
    )
...
def test_build_counterfactual_candidate_can_allowlist_candidate_answers(tmp_path: Path) -> None:
    ...
    answer_submission.write_text(
        json.dumps(
            {"architecture_summary": {}, "answers": [_submission_record(qid="q1", answer="A", page_id="doca_1")]}
        ),
        encoding="utf-8",
    )
    answer_raw.write_text(
...
def test_build_counterfactual_candidate_can_take_allowlisted_answers_without_page_drift(tmp_path: Path) -> None:
    answer_submission = tmp_path / "answer_submission.json"
    answer_raw = tmp_path / "answer_raw.json"
    answer_preflight = tmp_path / "answer_preflight.json"
    page_submission = tmp_path / "page_submission.json"
