...
def test_load_manifest_resolves_candidate_specs(tmp_path: Path) -> None:
                ...
                "candidates": [
                    {
                        ...
                        "changed_files": ["src/shafi/core/strict_answerer.py"],
    ...
    assert rows[0].changed_files == ["src/shafi/core/strict_answerer.py"]
...
def test_impact_router_routes_major_change_classes() -> None:
    strict = route_changed_files(["src/shafi/core/strict_answerer.py"], completed_packs=["strict_answerer_pack"])
    retrieval = route_changed_files(["src/shafi/core/retriever.py"], completed_packs=["page_localization_pack"])
    reingest = route_changed_files(["src/shafi/ingestion/parser.py"], completed_packs=["reingest_ocr_pack"])
    free_text = route_changed_files(
        ["src/shafi/prompts/llm/generator_system_complex.md"], completed_packs=["free_text_pack"]
    )
    config = route_changed_files(["src/shafi/config/settings.py"], completed_packs=["full_regression_pack"])
...
def test_apply_impact_router_blocks_candidate_when_required_pack_missing() -> None:
    ...
    _apply_impact_router(
        CandidateSpec(
            ...
            allowed_answer_qids=[],
            allowed_page_qids=[],
            changed_files=["src/shafi/core/retriever.py"],
            completed_packs=[],
            branch_class="doc_page_rerank_core",
            timeline_scope="active",
        ),
