...
from shafi.core.request_limiter import AsyncRequestLimiter
...
async def test_query_endpoint_streams_token_telemetry_and_done(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        routes.app_state, "query_limiter", AsyncRequestLimiter(concurrency_limit=100, min_interval_s=0.0), raising=False
    )
