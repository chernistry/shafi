...
from rag_challenge.core.proof_answerer import ProofAnswer
from rag_challenge.models import Citation, DocType, QueryComplexity, RankedChunk
...
def _build_builder() -> RAGPipelineBuilder:
    ...
    retriever.retrieve = AsyncMock(return_value=[_chunk(chunk_id="c0", text="The law applies."), _chunk(chunk_id="c1", text="Payment is due within 30 days.")])
    retriever.retrieve_with_retry = AsyncMock(return_value=[_chunk(chunk_id="c0", text="The law applies."), _chunk(chunk_id="c1", text="Payment is due within 30 days.")])
...
async def test_pipeline_proof_compiler_opt_in() -> None:
    ...
    with patch("rag_challenge.core.pipeline.get_stream_writer", return_value=lambda _event: None):
...
async def test_pipeline_proof_compiler_respects_runtime_coverage_gate() -> None:
    ...
    with (
        patch("rag_challenge.core.pipeline.get_stream_writer", return_value=lambda _event: None),
        patch(
            "rag_challenge.core.pipeline.generation_logic.ProofCarryingCompiler.compile",
