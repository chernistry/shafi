...
from shafi.core.embedding import EmbeddingClient, EmbeddingError
...
def mock_settings():
    ...
    with patch("shafi.core.embedding.get_settings", return_value=settings):
        yield settings
...
async def test_embed_query_uses_local_model_when_enabled():
    ...
    settings = SimpleNamespace(embedding=embedding)
    with (
        patch("shafi.core.embedding.get_settings", return_value=settings),
        patch(
            "shafi.core.embedding.importlib.import_module",
