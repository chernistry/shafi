...
async def test_generate_hypothetical_document_handles_none_content() -> None:
    ...
    with patch("rag_challenge.core.hyde.AsyncOpenAI") as mock_cls:
        mock_cls.return_value = mock_client

        from rag_challenge.core.hyde import generate_hypothetical_document

        result = await generate_hypothetical_document(
            "What is the penalty for late filing?", settings=mock_settings
        )
...
async def test_generate_hypothetical_document_returns_stripped_string() -> None:
    ...
    with patch("rag_challenge.core.hyde.AsyncOpenAI") as mock_cls:
        ...
        result = await generate_hypothetical_document(
            "What is the penalty?", settings=mock_settings
        )
