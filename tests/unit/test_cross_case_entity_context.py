...
from rag_challenge.models import DocType, RankedChunk, RetrievedChunk
...
def _make_pipeline(chunks_raw: list[RetrievedChunk]) -> MagicMock:
    ...
    from rag_challenge.core.pipeline.retrieval_seed_selection import case_ref_identity_score
    from rag_challenge.core.pipeline.support_query_primitives import normalize_support_text

    pipeline = MagicMock()
    pipeline.normalize_support_text.side_effect = normalize_support_text

    def _case_score(*, ref: str, chunk: RetrievedChunk | RankedChunk) -> int:  # noqa: F841
...
class TestEnsureCrossCaseEntityContext:
    ...
    def test_promotes_both_case_chunks_when_two_refs_present(self) -> None:
        ...
        from rag_challenge.core.pipeline.retrieval_boolean_handlers import (
    ...
    def test_no_op_when_single_case_ref(self) -> None:
        ...
        result = ensure_cross_case_entity_context(
            pipeline, query=query, reranked=reranked, retrieved=[chunk], top_n=3
        )
    ...
    def test_respects_top_n_limit(self) -> None:
        ...
        raw_chunks = [
            _retrieved(f"c{i}", f"doc-{i}", f"CA 00{i}/2025 X v Y") for i in range(5)
        ]
