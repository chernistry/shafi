...
def test_selective_icr_shadow_diagnostics_are_emitted():
    collector = TelemetryCollector(request_id="req-8")
    collector.set_selective_icr_shadow_diagnostics(
        used=True,
        model="selective_icr_heuristic",
        latency_ms=17,
        candidate_count=4,
        chunk_ids=["doc-a:0:0:a", "doc-b:1:0:b"],
        page_ids=["doc-a_1", "doc-b_2"],
        provider_exit=False,
    )

    payload = collector.finalize()

    assert payload.selective_icr_shadow_used is True
    assert payload.selective_icr_shadow_model == "selective_icr_heuristic"
    assert payload.selective_icr_shadow_latency_ms == 17
    assert payload.selective_icr_shadow_candidate_count == 4
    assert payload.selective_icr_shadow_chunk_ids == ["doc-a:0:0:a", "doc-b:1:0:b"]
    assert payload.selective_icr_shadow_page_ids == ["doc-a_1", "doc-b_2"]
    assert payload.selective_icr_shadow_provider_exit is False
...
def test_get_trained_page_scorer_page_ids_returns_stored_ids() -> None:
    collector = TelemetryCollector(request_id="req-scorer")
    assert collector.get_trained_page_scorer_page_ids() == []

    collector.set_trained_page_scorer_diagnostics(
        used=True,
        model_path="models/v6/model.joblib",
        page_ids=["doc1_1", "doc1_2", "doc2_3"],
    )
    assert collector.get_trained_page_scorer_page_ids() == ["doc1_1", "doc1_2", "doc2_3"]
