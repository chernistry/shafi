...
def test_run_experiment_gate_script_reports_and_appends_ledger(tmp_path: Path) -> None:
    ...
    assert "## Staged Eval" in report
    assert "- Blocked stage: `none`" in report
    assert "- Should run Stage 3: `True`" in report
    ...
    assert report_payload["staged_eval"]["blocked_stage"] is None
    assert report_payload["staged_eval"]["should_run_stage3"] is True
    ...
    assert experiments[0]["blocked_stage"] is None
    assert experiments[0]["should_run_stage3"] is True


def test_run_experiment_gate_blocks_before_stage3_on_impact_canary_failure(tmp_path: Path) -> None:
    baseline_submission = tmp_path / "baseline_submission.json"
    candidate_submission = tmp_path / "candidate_submission.json"
    baseline_raw = tmp_path / "baseline_raw.json"
    candidate_raw = tmp_path / "candidate_raw.json"
    ...
    subprocess.run(
        [
            ...
            "scripts/run_experiment_gate.py",
            "--label",
            "candidate-canary-blocked",
            "--baseline-label",
            "baseline-canary-blocked",
            "--baseline-submission",
            str(baseline_submission),
            "--candidate-submission",
            str(candidate_submission),
            ...
            "blocked",
            "--impact-canary-reason",
            "page-localization canary regressed",
            "--impact-canary-pack",
            "page_localization_pack",
            "--seed-qid",
            "q1",
            "--out",
            str(report_path),
    ...
    assert "- Recommendation: `NO_SUBMIT`" in report
    assert "- Blocked stage: `stage1_impact_routed_canary`" in report
    assert "- Block reason: `page-localization canary regressed`" in report
    assert "- Should run Stage 3: `False`" in report
    assert payload["staged_eval"]["blocked_stage"] == "stage1_impact_routed_canary"
    assert payload["staged_eval"]["block_reason"] == "page-localization canary regressed"
    assert payload["staged_eval"]["should_run_stage3"] is False
                ...
                "answers": [
                    {
                        ...
                        "answer": ["Alpha LLC"],
                        "telemetry": {
                            "retrieval": {"retrieved_chunk_pages": [{"doc_id": "doc-old", "page_numbers": [1]}]}
                        },
                    }
                ]
            }
                ...
                "answers": [
                    {
                        ...
                        "answer": ["Alpha LLC"],
                        "telemetry": {
                            "retrieval": {"retrieved_chunk_pages": [{"doc_id": "doc-new", "page_numbers": [1]}]}
                        },
                    }
                ]
            }
                ...
                "answers": [
                    {
                        ...
                        "answer": False,
                        "telemetry": {
                            "retrieval": {"retrieved_chunk_pages": [{"doc_id": "doc-old", "page_numbers": [3]}]}
                        },
                    }
                ]
            }
                ...
                "answers": [
                    {
                        ...
                        "answer": False,
                        "telemetry": {
                            "retrieval": {"retrieved_chunk_pages": [{"doc_id": "doc-live", "page_numbers": [1]}]}
                        },
                    }
                ]
            }
