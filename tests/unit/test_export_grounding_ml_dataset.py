...
from shafi.ml.grounding_dataset import export_grounding_ml_dataset, split_grounding_ml_rows
...
def test_split_grounding_ml_rows_is_deterministic(tmp_path: Path) -> None:
    from shafi.ml.grounding_dataset import GroundingMlRow, PageRetrievalFeatureRecord, SupportFactFeatureRecord
...
def test_export_grounding_ml_dataset_writes_expected_files(tmp_path: Path) -> None:
    ...
    _write_json(
        benchmark_path, {"cases": [{"question_id": "qid-1", "gold_page_ids": ["law_16"], "trust_tier": "trusted"}]}
    )
    ...
    train_rows = [
        json.loads(line) for line in (output_dir / "train.jsonl").read_text(encoding="utf-8").splitlines() if line
    ]
    dev_rows = [
        json.loads(line) for line in (output_dir / "dev.jsonl").read_text(encoding="utf-8").splitlines() if line
    ]
