...
class TestShouldRunConsensus:
    def test_false_when_flag_off(self) -> None:
        ...
        assert should_run_consensus(
            answer_type="boolean", extracted=False, settings=settings,  # type: ignore[arg-type]
        ) is False

    def test_false_when_extracted(self) -> None:
        settings = _StubSettings(enable_answer_consensus=True)
        assert should_run_consensus(
            answer_type="boolean", extracted=True, settings=settings,  # type: ignore[arg-type]
        ) is False

    def test_false_for_date_type(self) -> None:
        settings = _StubSettings(enable_answer_consensus=True)
        assert should_run_consensus(
            answer_type="date", extracted=False, settings=settings,  # type: ignore[arg-type]
        ) is False
    ...
    def test_true_for_number_unextracted(self) -> None:
        settings = _StubSettings(enable_answer_consensus=True)
        assert should_run_consensus(
            answer_type="number", extracted=False, settings=settings,  # type: ignore[arg-type]
        ) is True
