...
def test_bm25_sparse_encoder_defaults_cache_dir_to_workspace_cache(tmp_path: Path) -> None:
    ...
    with (
        patch("shafi.core.sparse_bm25.Path.cwd", return_value=tmp_path),
...
def test_bm25_sparse_encoder_falls_back_when_custom_cache_dir_is_unavailable(tmp_path: Path) -> None:
    ...
    with (
        patch("shafi.core.sparse_bm25.Path.cwd", return_value=tmp_path),
        patch("shafi.core.sparse_bm25.BM25SparseEncoder._prepare_cache_dir", side_effect=_fake_prepare),
        patch("fastembed.sparse.SparseTextEmbedding", _FakeSparseTextEmbedding),
    ):
...
def test_bm25_sparse_encoder_silently_maps_legacy_container_cache_to_workspace(tmp_path: Path) -> None:
    ...
    with (
        patch("shafi.core.sparse_bm25.Path.cwd", return_value=tmp_path),
        patch("shafi.core.sparse_bm25.logger.warning") as warning_mock,
        patch("fastembed.sparse.SparseTextEmbedding", _FakeSparseTextEmbedding),
    ):
