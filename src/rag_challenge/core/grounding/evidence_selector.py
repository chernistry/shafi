...
from rag_challenge.core.grounding.authority_priors import select_authoritative_single_page
from rag_challenge.core.grounding.condition_audit import ConditionAuditResult, audit_candidate_pages
...
from rag_challenge.core.grounding.page_semantic_lane import (
...
from rag_challenge.core.grounding.relevance_verifier import BoundedPageRelevanceVerifier
...
if TYPE_CHECKING:
    ...
    from rag_challenge.config.settings import PipelineSettings, VerifierSettings
...
class GroundingEvidenceSelector:
        ...
        if (
            ...
            self._relevance_verifier = BoundedPageRelevanceVerifier(
                ...
                min_confidence=float(
                    getattr(self._settings, "grounding_relevance_verifier_min_confidence", 0.7)
                ),
            )
    ...
    async def select_page_ids(
        ...
        *,
        ...
        if (
            ...
            and escalation.allow_relevance_verifier
            and self._relevance_verifier is not None
        ):
            verifier_pages = self._build_relevance_verifier_candidates(
                ordered_pages=active_page_candidates,
                selected_pages=final_selected_pages,
                max_candidates=int(
                    getattr(self._settings, "grounding_relevance_verifier_max_candidates", 3)
                ),
            )
            if verifier_pages:
    ...
    def _select_minimal_pages(
        ...
        *,
        ...
        if scope.scope_mode not in {ScopeMode.SINGLE_FIELD_SINGLE_DOC, ScopeMode.COMPARE_PAIR, ScopeMode.BROAD_FREE_TEXT}:
            return safe_page_ids
            ...
            if compare_panel:
                add_candidate(
                    PortfolioCandidate(
                        ...
                        reasons=tuple(
                            f"{panel.doc_id}:{panel.case_ref or 'context_doc'}"
                            for panel in compare_panel
                        ),
                    )
                )
