...
from rag_challenge.core.grounding.authority_priors import (
...
def rerank_pages_with_shadow_signal(query: str, pages: list[RetrievedPage]) -> list[RetrievedPage]:
    ...
    rescored = [
        page.model_copy(update={"score": page.score + shadow_signal_score(query, page)})
        for page in pages
    ]
    return sorted(
        rescored,
...
def _supports_single_doc_pair(query: str) -> bool:
    ...
    return bool(_SCHEDULE_QUERY_RE.search(query) or _TITLE_ARTICLE_QUERY_RE.search(query) or _PARTY_QUERY_RE.search(query))
