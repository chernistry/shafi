...
class ProofCarryingCompiler:
    ...
    def filter_verified_claims(self, claims: Sequence[Claim]) -> tuple[list[Claim], list[Claim]]:
        ...
        dropped: list[Claim] = []
        for claim in claims:
            if claim.support_type in {SupportType.DIRECTLY_STATED, SupportType.INFERRED, SupportType.COMPUTED} and claim.evidence_spans:
    ...
    def compile_boolean(self, verified: Sequence[Claim], contract: QueryContract | None) -> ProofAnswer:
        ...
        yes_like = any(claim.claim_text.casefold().startswith(("yes", "true", "it applies", "applies")) for claim in verified)
        no_like = any(
            any(term in claim.claim_text.casefold() for term in ("no ", "not ", "does not", "did not", "without", "doesn't"))
    ...
    def attach_provenance(self, answer_text: str, claims: Sequence[Claim]) -> dict[int, list[str]]:
        ...
        sentences = [segment for segment in re.split(r"(?<=[.!?])\s+", str(answer_text or "").strip()) if segment.strip()]
