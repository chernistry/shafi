...
def build_filter(
    *,
    ...
    exclude_ids = [
        did.strip()
        for did in (list(must_not_doc_ids) if must_not_doc_ids is not None else [])
        if str(did).strip()
    ]
    if exclude_ids:
        must_not.append(
...
def expand_doc_ref_variants(refs: list[str] | tuple[str, ...]) -> list[str]:
    ...
    for raw in refs:
        ...
        if instrument_match is not None:
            ...
            family = "Law" if family_raw.casefold() == "law" else "Regulations" if family_raw.casefold().startswith("regulation") else "Rules"
