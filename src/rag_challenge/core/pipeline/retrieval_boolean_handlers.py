...
from rag_challenge.core.grounding.scope_policy import extract_case_refs
...
def ensure_cross_case_entity_context(
    pipeline: RAGPipelineBuilder,
    *,
    query: str,
    reranked: list[RankedChunk],
    ...
    """Ensure context covers all case documents when query references ≥2 case IDs.

    For cross-case entity questions (e.g. "Is there any main party that appeared
    in both cases CFI 035/2025 and CFI 081/2023?"), the dense retrieval may return
    chunks only from the semantically-nearest case.  This function detects ≥2 case
    refs in the query and promotes the best-matching chunk from each case doc to the
    front of the context window, so the LLM can compare both case files.

    Args:
        pipeline: The pipeline builder instance providing scoring helpers.
        query: Raw user question text.
        reranked: Reranked chunks from the normal retrieval path (answer-path context).
        retrieved: Full set of retrieved chunks before reranking (broader candidate pool).
        top_n: Desired context size.

    Returns:
        Reordered context with at least one chunk per referenced case in leading slots,
        followed by remaining reranked chunks up to top_n.
    """
    if top_n <= 0 or not retrieved:
        return reranked[: max(0, int(top_n))]
    ...
    # For each referenced case, find the best chunk from that case doc.
    for ref in case_refs:
        ...
        if doc_id:
            matched_doc_ids.add(doc_id)
        if best_raw.chunk_id not in seen_chunk_ids:
            selected.append(reranked_by_id.get(best_raw.chunk_id) or pipeline.raw_to_ranked(best_raw))
            seen_chunk_ids.add(best_raw.chunk_id)

    # Fill remaining slots with standard reranked order.
    for chunk in reranked:
        if chunk.chunk_id in seen_chunk_ids:
            continue
        selected.append(chunk)
