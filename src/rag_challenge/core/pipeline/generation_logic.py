...
from rag_challenge.core.claim_graph import ClaimGraphBuilder
...
class GenerationLogicMixin:
    async def _generate(self, state: RAGState) -> dict[str, object]:
        ...
        if (
            answer_type in _eqa_target_types
            and bool(getattr(self._settings.pipeline, "enable_extractive_qa", False))
        ):
            from rag_challenge.core.isaacus_eqa import call_isaacus_eqa
        ...
        if not answer and answer_type in strict_types and bool(getattr(self._settings.pipeline, "strict_types_extraction_enabled", True)):
        ...
        if answer_type in strict_types:
            ...
            from rag_challenge.core.pipeline.answer_quality_gate import (
            ...
            if should_run_consensus(
                ...
                from rag_challenge.core.pipeline.answer_consensus import AnswerConsensus
            ...
            if self._is_unanswerable_strict_answer(answer):
                ...
                if not cited_ids:
                    if context_chunk_ids:
                        ...
                        # on short/boolean questions.  F-beta=2.5 weights recall 6.25×:
                        # citing top-N context pages maximises recall at low precision cost.
                        _strict_page_budget = int(
                            getattr(self._settings.pipeline, "strict_types_context_top_n", 5)
                        )
            ...
            if (
                ...
                _verify_model = str(
                    getattr(self._settings.pipeline, "citation_verification_model", "gpt-4.1-mini")
                )
