...
from rag_challenge.core.bm25_retriever import BM25Retriever, load_alias_map
from rag_challenge.core.circuit_breaker import CircuitBreaker
from rag_challenge.core.citation_graph import CitationGraphExpander
from rag_challenge.core.classifier import QueryClassifier
from rag_challenge.core.entity_registry import EntityRegistry
from rag_challenge.core.hyde import generate_hypothetical_document
from rag_challenge.core.retriever_filters import (
    build_filter,
    build_sparse_query,
    coerce_float,
    coerce_int,
        ...
        *,
        ...
        if result is None:
            ...
            if hyde_enabled and query_vector is None:
                ...
                hypo_doc_task = asyncio.create_task(
                    generate_hypothetical_document(str(query or ""), get_settings())
                )
                original_embed_task = asyncio.create_task(
                    self._embedder.embed_query(embedding_query)
                )
        ...
        if bool(getattr(self._pipeline_settings, "enable_doc_title_boost", False)) and extracted_refs and chunks:
            ...
            for chunk in chunks:
                ...
                matched = norm_title in title_boost_refs or any(
                    norm_title.startswith(ref) or ref in norm_title
                    for ref in title_boost_refs
                )
                if matched:
        ...
        if bool(getattr(self._pipeline_settings, "dedup_duplicate_docs", False)) and chunks:
            ...
            chunks, dedup_removed = _dedup_duplicate_docs(
                chunks, min_title_length=min_title_len
            )
        ...
        *,
        ...
        try:
            variant_vectors: list[list[float]] = [  # type: ignore[assignment]
                vec for vec in await asyncio.gather(
                    *[self._embedder.embed_query(v) for v in variant_queries]
                )
            ]
        except Exception as exc:
        ...
        if (
            ...
            if segment_hits:
                ...
                promoted_chunks = [
                    ...
                    if (score := self._score_segment_chunk(
                        chunk=chunk,
