...
def compute_drift(observations: list[RunObservation]) -> DriftRecord:
    ...
    page_variants_raw = list(dict.fromkeys(json.dumps(sorted(obs.used_page_ids), ensure_ascii=False) for obs in observations))
...
def _is_field_question(question: str) -> bool:
    lowered = question.lower()
    return any(term in lowered for term in ("who is", "who was", "date", "issued by", "law no", "claimant", "judge", "party", "amount"))
...
def _is_temporal_question(question: str) -> bool:
    lowered = question.lower()
    return any(term in lowered for term in ("effective", "commence", "commencement", "amend", "supersed", "repeal", "replace"))
