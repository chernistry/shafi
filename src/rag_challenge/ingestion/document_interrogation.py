...
def parse_document_interrogation_json(raw_json: str) -> DocumentInterrogationRecord:
    ...
    return normalized_record.model_copy(
        update={"compact_shadow_text": build_compact_shadow_text(normalized_record)}
    )
...
def load_document_interrogation_inputs(path: Path) -> list[DocumentInterrogationInput]:
    ...
    if path.suffix.lower() == ".jsonl":
        return [
            DocumentInterrogationInput.model_validate_json(line)
