...
from rag_challenge.core.law_notice_support import extract_enactment_authority, extract_enactment_date
from rag_challenge.ingestion.manual_domain_overrides import apply_manual_domain_override, get_manual_domain_override
from rag_challenge.ingestion.parser import DocumentParser
from rag_challenge.models import DocType, ParsedDocument
...
class CorpusCompiler:
        ...
        *,
        ...
        field_page_ids = self._field_page_ids(
            parsed_doc,
            {
                ...
                "law_number": [value for value in (law_number, f"Law No. {law_number} of {year}" if law_number and year else "") if value],
    ...
    def extract_case_metadata(self, parsed_doc: ParsedDocument) -> CaseObject:
        ...
        role_parties = [
            CaseParty(name=name, role=role)
            for role, name in self._extract_role_parties(parsed_doc.full_text)
        ]
    ...
    def _build_links(self, registry: CorpusRegistry) -> list[ProvenanceLink]:
        ...
        title_lookup.update({law.short_title.casefold(): law.doc_id for law in registry.laws.values() if law.short_title})
        title_lookup.update(
            {
                alias.casefold(): law.doc_id
                for law in registry.laws.values()
    ...
    def _extract_article_tree(self, parsed_doc: ParsedDocument) -> list[ArticleNode]:
        ...
        for index, section in enumerate(parsed_doc.sections, start=1):
            ...
            node_page_ids = [self._section_page_id(parsed_doc, section.section_path)] if section.section_path.startswith("page:") else [fallback_page]
            nodes.append(
                ArticleNode(
    ...
    def _extract_caption_parties(title: str) -> list[CaseParty]:
        ...
        if CorpusCompiler._is_plausible_party_value(left):
            parties.append(CaseParty(name=left, role=CorpusCompiler._caption_role_or_default(left_raw, default="claimant")))
    ...
    def _merge_parties(parties: list[CaseParty]) -> list[CaseParty]:
        ...
        for party in parties:
            ...
            if not key or key in seen:
                continue
            seen.add(key)
            merged.append(CaseParty(name=cleaned_name, role=normalized_role, canonical_entity_id=party.canonical_entity_id))
        return merged
