...
from rag_challenge.core.legal_title_family import (
...
if TYPE_CHECKING:
    ...
    from rag_challenge.models import Chunk, LegalSegment, PageMetadata, ParsedDocument
...
_BRACKET_CASE_RE = re.compile(
    r"\[(\d{4})\]\s+DIFC\s+(CFI|CA|ARB|SCT|TCD|ENF|DEC)\s+(\d+)", re.IGNORECASE
)
                        ...
                        if not chunks:
                            ...
                            if segments:
                                ...
                                for idx, chunk in enumerate(augmented):
                                    ...
                                    augmented[idx] = chunk.model_copy(update={"canonical_entity_ids": merged_entity_ids})
    ...
    def _compile_segments_for_doc(
        ...
        *,
        ...
        if not bool(getattr(self._settings.ingestion, "build_segment_collection", False)):
            return []
        from rag_challenge.ingestion.legal_segments import SegmentCompiler
