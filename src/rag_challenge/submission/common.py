...
def coerce_str_list(value: object) -> list[str]:
    ...
    return [text for item in items if item is not None and (text := str(item).strip())]


def select_submission_used_pages(telemetry: dict[str, object]) -> list[str]:
    # Fallback chain must mirror eval/sources.py select_used_pages:
    # used_page_ids → used_chunk_ids → cited_page_ids → cited_chunk_ids → []
    used_pages = coerce_str_list(telemetry.get("used_page_ids"))
    if not used_pages:
        used_chunk_ids = coerce_str_list(telemetry.get("used_chunk_ids"))
        used_pages = chunk_ids_to_page_ids(used_chunk_ids) if used_chunk_ids else []
    if used_pages:
        return used_pages
    ...
    if not cited_pages:
        ...
        cited_pages = chunk_ids_to_page_ids(cited_chunk_ids) if cited_chunk_ids else []
    if cited_pages:
        return cited_pages
...
def normalize_date_answer(answer: str) -> str | None:
    ...
    if not text:
        return None
    # Strip ordinal suffixes ("15th" → "15") so date regexes match.
    text = _ORDINAL_SUFFIX_RE.sub(r"\1", text)
    # Strip "day of" in legal phrasing ("the 15 day of March" → "the 15 March").
    text = re.sub(r"(\d{1,2})\s+day\s+of\s+", r"\1 ", text)
    # Normalize "X of Month" → "X Month" for textual date regexes.
    text = re.sub(r"(\d{1,2})\s+of\s+([A-Za-z])", r"\1 \2", text)
...
def split_submission_sentences(text: str) -> list[str]:
    ...
    for part in merged_parts:
        if _SENTENCE_END_RE.search(part):
            ...
            continue
        # Include one non-punctuated segment (likely a sentence missing its
        # final period) then stop — avoids dropping valid content while
        # still stopping before clearly truncated fragments.
        sentences.append(part)
        break
    return sentences
...
def normalize_free_text_answer(answer: str) -> str:
    ...
    if segments:
        # Strip preamble/filler from each segment to maximize budget.
        from rag_challenge.core.pipeline.free_text_cleanup import condense_free_text as _cft

        segments = [s for raw in segments if (s := _cft(raw).strip())]
    ...
    # Strip verbose preamble/filler to maximize content within 280-char budget.
    from rag_challenge.core.pipeline.free_text_cleanup import condense_free_text

    text = condense_free_text(text)
    ...
    if sentences:
        ...
        while capped_sentences:
            ...
            if len(candidate) <= _FREE_TEXT_LIMIT:
                # When we had to drop a sentence to fit, try filling
                # remaining budget with a truncated version of the dropped sentence.
                n_used = len(capped_sentences)
                dropped_some = n_used < min(3, len(sentences))
                if dropped_some and n_used < len(sentences) and len(candidate) < _FREE_TEXT_LIMIT - 20:
                    remaining = _FREE_TEXT_LIMIT - len(candidate) - 1  # -1 for joining space
                    tail = truncate_at_word_boundary(sentences[n_used], limit=remaining)
                    if tail:
                        extended = f"{candidate} {tail}"
                        # Full tails get validated for dangling markers;
                        # truncated tails (ending "...") bypass _trim_fragment_tail
                        # which would strip the "..." and lose the added content.
                        if not tail.endswith("..."):
                            extended = _trim_fragment_tail(extended)
                        if extended and len(extended) <= _FREE_TEXT_LIMIT:
                            return extended
                return candidate
            capped_sentences = capped_sentences[:-1]

    # Last resort: truncate but ENSURE the result ends with a complete sentence.
    # Mid-sentence truncation kills Completeness+Clarity in LLM judge scoring.
    truncated = truncate_at_word_boundary(text, limit=_FREE_TEXT_LIMIT)
    trimmed = _trim_fragment_tail(truncated)
    # If trimming removed everything (single long sentence), prefer the trimmed
    # fragment over nothing — but try to end cleanly.
    if trimmed:
        return trimmed
    # If we still have "..." ending, try to find last sentence boundary
    if truncated.endswith("..."):
        clean = truncated[:-3].rstrip()
        last_period = max(clean.rfind(". "), clean.rfind(".\u201d"), clean.rfind(".)"))
        if last_period > len(clean) // 3:  # at least 1/3 of content preserved
...
def classify_unanswerable_answer(answer: str, answer_type: str) -> tuple[bool, bool]:
    ...
    # Treat punctuation-only or whitespace-only strings as unanswerable.
    stripped_alpha = re.sub(r"[^A-Za-z0-9]", "", answer_normalized)
    strict_types = {"boolean", "number", "date", "name", "names"}
    is_unanswerable_strict = answer_type_key in strict_types and (
        answer_normalized in {"", "null", "none"} or not stripped_alpha
    )
    _in_sources = "provided sources" in answer_normalized or "provided documents" in answer_normalized
    is_unanswerable_free_text = answer_type_key == "free_text" and (
        ...
        or (
            _in_sources
            and (
                answer_normalized.startswith("there is no information on ")
                or answer_normalized.startswith("there is no information about ")
                or answer_normalized.startswith("there is no information in the provided")
            )
        )
    )
