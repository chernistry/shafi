...
from rag_challenge.ml.hard_negative_miner import HardNegativeMiner, NegativeCandidate, NegativeStrategy
...
if TYPE_CHECKING:
    ...
    from rag_challenge.ml.synthetic_qa_factory import BridgeFactRecord, SyntheticQAExample
...
class TeacherLabelBuilder:
    ...
    def build_from_grounding_rows(self, rows: Sequence[GroundingMlRow]) -> list[TrainingTriple]:
        ...
        for row in rows:
            ...
            gold_doc_ids = [self._page_doc_ids.get(page_id, page_id.rpartition("_")[0]) for page_id in positive_page_ids]
    ...
    def build_from_synthetic_qa(self, examples: Sequence[SyntheticQAExample]) -> list[TrainingTriple]:
        ...
        for example in examples:
            if not example.gold_page_ids:
                continue
            positive_text_by_page = {
                page_id: self._page_texts.get(page_id, example.answer)
                for page_id in example.gold_page_ids
            }
            negatives = [
                NegativeCandidate(
                    page_id=page_id,
    ...
    def build_from_bridge_facts(self, bridge_facts: Sequence[BridgeFactRecord]) -> list[TrainingTriple]:
        ...
        for record in bridge_facts:
            ...
            positive_text_by_page = {
                page_id: self._page_texts.get(page_id, record.answer)
    ...
    def _triples_from_positive_pages(
        ...
        *,
        ...
        for positive_page_id in positive_page_ids:
            positive_text = positive_text_by_page.get(positive_page_id, self._page_texts.get(positive_page_id, "")).strip()
...
def build_page_texts_from_registry(registry: CorpusRegistry) -> tuple[dict[str, str], dict[str, str], dict[str, list[str]]]:
