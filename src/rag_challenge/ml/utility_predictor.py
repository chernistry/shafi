...
from rag_challenge.core.retrieval_utility import BundleSnapshot, RetrievalUtilityPredictor, UtilityPredictorArtifact
...
class UtilityPredictorTrainer:
    ...
    def cross_validate(self, examples: Sequence[UtilityTrainingExample], *, folds: int = 5) -> UtilityMetrics:
        ...
        vectorizer = DictVectorizer(sparse=True)
        x = vectorizer.fit_transform([RetrievalUtilityPredictor().extract_features(example.snapshot) for example in examples])
...
def build_feature_importance(fitted: FittedUtilityPredictor, *, top_n: int = 12) -> dict[str, list[dict[str, float | str]]]:
    ...
    return {
        "positive": [
            {"feature": feature_names[index], "weight": coefficients[index]}
