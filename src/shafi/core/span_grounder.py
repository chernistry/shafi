...
class SpanGrounder:
    ...
    def match_to_segments(
        ...
        for span in spans:
            ...
            for segment in segments:
                ...
                page_ids = [
                    str(page_id).strip() for page_id in getattr(segment, "page_ids", []) if str(page_id).strip()
                ]
    ...
    def _token_set(text: str) -> set[str]:
        ...
        return {
            token.casefold() for token in re.findall(r"[A-Za-z0-9][A-Za-z0-9./-]*", str(text or "")) if len(token) > 1
        }
