...
def doc_family_collapse_candidate_score(
    pipeline: RAGPipelineBuilder, *, query: str, chunk: RetrievedChunk | RankedChunk
) -> tuple[int, int, float]:
...
def collapse_doc_family_crowding_context(
    ...
    *,
    ...
    for raw in retrieved:
        ...
        if current is None or pipeline.doc_family_collapse_candidate_score(
            query=query, chunk=raw
        ) > pipeline.doc_family_collapse_candidate_score(
            query=query,
            chunk=current,
        ):
...
def augment_strict_context_chunks(
    ...
    *,
    ...
    rescue_candidates = [raw for raw in retrieved if pipeline.page_num(str(raw.section_path or "")) == requested_page]
    if not rescue_candidates and requested_page == 2:
        rescue_candidates = [raw for raw in retrieved if pipeline.page_num(str(raw.section_path or "")) in {1, 2}]
