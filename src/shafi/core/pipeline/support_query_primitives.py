...
def paired_support_question_refs(pipeline: RAGPipelineBuilder, query: str) -> list[str]:
    """Return paired refs for multi-document boolean retrieval.

    Handles two pairing strategies, in priority order:

    1. **Case-ref pairing** (cross-case boolean ~102Q): when ≥2 DIFC case
       references are found (e.g. "CFI 001/2020" and "CA 003/2021"), return
       them as a paired unit so retrieval fetches pages from BOTH case docs.
       Falls through to strategy 2 only when fewer than 2 case refs exist.

    2. **Law title+number pairing** (original behaviour): pairs regulation
       title strings with their matching "Law No. X of Y" identifiers.
       Falls back to ``support_question_refs`` when pairing is not possible.

    Args:
        pipeline: Active RAGPipelineBuilder instance supplying helper methods.
        query: Raw user question text.

    Returns:
        Ordered list of reference strings to use for paired retrieval.
    """
    # --- Strategy 1: cross-case boolean pairing ---
    # Must run BEFORE title/law-number logic so case refs are not dropped.
    case_refs = extract_case_refs(query)
    if len(case_refs) >= 2:
        return case_refs

    # --- Strategy 2: law title + law number pairing (original logic) ---
    title_refs = [
...
def ref_has_criterion_support(
    ...
    *,
    ...
    if _is_common_elements_query(query):
        ...
        return any(
            pipeline.common_elements_evidence_score(
                str(getattr(chunk, "text", "") or ""), interpretation_sections=interpretation_sections
            )
            > 0
...
def build_chunk_snippet_map(
    pipeline: RAGPipelineBuilder, chunks: Sequence[RetrievedChunk | RankedChunk]
) -> dict[str, str]:
    ...
    *,
    ...
    for chunk_id in ordered_ids:
        ...
        if previous_page_chunks:
            ...
            if pipeline.page_text_looks_like_continuation_tail(previous_text) or (
        ...
        if next_page_chunks:
            ...
            if pipeline.page_text_looks_like_continuation_tail(current_text) or (
