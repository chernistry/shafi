...
def extract_citation_pages(
    *,
    ...
    def _terms(text: str) -> set[str]:
        return {w for w in re.sub(r"[^\w]", " ", text.lower()).split() if w and w not in stopwords and len(w) > 2}
    ...
    # F-beta 2.5: recall is 6.25x more important than precision.
    # Missing 1 gold page costs ~46% G; adding 1 wrong page costs ~6.5%.
    # Aggressive caps and low thresholds maximize recall at minimal precision cost.
    thresholds = {
        "boolean": (0.04, 10),
        "number": (0.10, 8),
        "date": (0.10, 8),
    ...
    threshold, max_pages = thresholds.get(answer_type, (0.04, 10))
...
def rerank_support_pages_within_selected_docs(
    ...
    *,
    ...
    strict_single_doc_like = (
        normalized_answer_type in {"boolean", "name", "names", "date", "number"} and not compare_like
    )
    metadata_like = (
    ...
    if strict_single_doc_like:
        # With F-beta 2.5, recall is 6.25x more important than precision.
        # Returning only the #1 page destroys recall when gold spans 2+ pages.
        # Return top-2 pages: missing 1 gold page costs ~46% G, while adding
        # 1 wrong page costs only ~6.5%.
        top_chunk_ids: list[str] = []
        for sp in scored_pages[:2]:
            sp_doc_id, _, sp_page_raw = sp.page_id.rpartition("_")
            if not sp_doc_id or not sp_page_raw.isdigit():
                continue
            chunk_id = pipeline.best_support_chunk_id_for_doc_page(
                doc_id=sp_doc_id,
                page_num=int(sp_page_raw),
                context_chunks=context_chunks,
            )
            if chunk_id and chunk_id not in top_chunk_ids:
                top_chunk_ids.append(chunk_id)
        return top_chunk_ids if top_chunk_ids else ordered_used_ids

    selected_pages = select_top_pages_per_doc(
        scored_pages=scored_pages,
        doc_order=doc_order,
        per_doc_pages=2 if (metadata_page_family_query or compare_like) else 1,
    )
...
def named_metadata_requires_support_union(pipeline: RAGPipelineBuilder, query: str) -> bool:
    ...
    multiple_named_refs = " and " in q and (
...
def apply_support_shape_policy(
    ...
    *,
    ...
    """
    ordered_ids = list(
        dict.fromkeys(str(chunk_id).strip() for chunk_id in [*cited_ids, *support_ids] if str(chunk_id).strip())
    )
    ...
    compare_shape = (
        len(compare_refs) >= 2
        and kind in {"boolean", "name", "number", "date"}
        and (
            kind == "boolean"
            or _is_case_issue_date_name_compare_query(query, answer_type=answer_type)
            or "same year" in q_lower
    ...
    costs_query = (
        kind == "free_text" and _is_case_outcome_query(query) and ("cost" in q_lower or "final ruling" in q_lower)
    )
    if costs_query:
