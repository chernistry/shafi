...
def select_seed_chunk_id(
    pipeline: RAGPipelineBuilder, chunks: list[RetrievedChunk], seed_terms: list[str]
) -> str | None:
...
def boolean_year_seed_chunk_score(
...
def boolean_admin_seed_chunk_score(
    pipeline: RAGPipelineBuilder, *, ref: str, chunk: RetrievedChunk | RankedChunk
) -> int:
...
def ref_doc_family_consistency_adjustment(
    pipeline: RAGPipelineBuilder, *, ref: str, chunk: RetrievedChunk | RankedChunk
) -> int:
    ...
    score = 0
    law_pairs = {(int(match.group(1)), match.group(2)) for match in _LAW_NO_REF_RE.finditer(identity_blob)}
    if target_pair in law_pairs:
        score += 140

    foreign_pairs = {pair for pair in law_pairs if pair != target_pair and pair[1] != target_pair[1]}
    if foreign_pairs:
...
def is_consolidated_or_amended_family_chunk(
    pipeline: RAGPipelineBuilder, *, chunk: RetrievedChunk | RankedChunk
) -> bool:
