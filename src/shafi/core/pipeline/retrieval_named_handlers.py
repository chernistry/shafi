...
def named_commencement_title_match_score(
...
def named_amendment_clause_score(pipeline: RAGPipelineBuilder, *, query: str, ref: str, text: str) -> int:
    ...
    ref_terms = {token for token in pipeline.support_terms(ref) if token not in _SUPPORT_STOPWORDS and len(token) > 2}
    if ref_terms:
...
def ensure_named_amendment_context(
    ...
    *,
    ...
    best_anchor_score = 0
    for raw in retrieved:
        score = pipeline.named_commencement_title_match_score(
            amendment_ref, raw
        ) + pipeline.named_amendment_clause_score(
            query=query,
            ref=amendment_ref,
            text=str(getattr(raw, "text", "") or ""),
        )
