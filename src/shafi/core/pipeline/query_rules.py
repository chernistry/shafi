...
def _is_broad_enumeration_query(query: str) -> bool:
    ...
    if not q or not _is_enumeration_query(q):
        return False
    return q.startswith(
        ("which laws", "which regulations", "which documents", "which cases", "list all", "identify all", "name all")
    )
...
def _is_case_outcome_query(query: str) -> bool:
    ...
    if "final ruling" in q or "court of appeal rule" in q:
        return True
    return ("outcome" in q or "result" in q) and ("application" in q or "appeal" in q or "order" in q)
...
def _is_named_reference_enumeration_query(query: str) -> bool:
    q = (query or "").strip().lower()
    return _is_broad_enumeration_query(query) and any(
        term in q for term in ("mention", "mentions", "reference", "references")
    )
...
def _is_named_amendment_query(query: str) -> bool:
    ...
    return ref_count >= 1 and "enact" in q and ("what law did it amend" in q or "what laws did it amend" in q)
