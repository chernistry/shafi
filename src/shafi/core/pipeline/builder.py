...
from shafi.core.applicability_engine import ApplicabilityEngine
from shafi.core.compare_engine import CompareEngine
...
from shafi.core.rerank_instructions import build_rerank_instruction
from shafi.core.retrieval_escalation import RetrieverEscalationPolicy
from shafi.core.strict_answerer import StrictAnswerer
from shafi.models import QueryComplexity, RetrievedChunk
...
class RAGPipelineBuilder(OrchestrationLogicMixin, RetrievalLogicMixin, SupportLogicMixin, GenerationLogicMixin):
    ...
    async def _retrieve(self, state: RAGState) -> dict[str, object]:
        ...
        if doc_refs and is_boolean and is_appeal_query(str(state.get("query") or "")):
            doc_refs = self._appeal_detector.expand_doc_refs(doc_refs, query=str(state.get("query") or ""))
                            ...
                            for row in outcome_results:
                                ...
                                for chunk in row:
                                    ...
                                    if existing is None or chunk.score > existing.score:
                                        merged[chunk.chunk_id] = chunk
                            ranked = sorted(
                                merged.values(), key=lambda chunk: (-chunk.score, chunk.doc_id, chunk.chunk_id)
                            )
                            seed_set = set(must_include_chunk_ids)
                            ordered: list[RetrievedChunk] = []
                            for chunk_id in must_include_chunk_ids:
        ...
        if title_refs and bool(getattr(self._settings.pipeline, "doc_ref_multi_retrieve", True)):
            ...
            if should_title_retrieve and title_refs:
                ...
                for title, row in zip(title_refs[:cap_titles], title_results, strict=False):
                    if row:
                        seed = (
                            self._select_targeted_title_seed_chunk_id(
                                query=state["query"],
                                answer_type=answer_type,
                                ref=title,
                                chunks=row,
        ...
        if answer_type == "free_text" and (
            ...
            if missing_refs:
                ...
                if merged:
                    limit = int(self._settings.reranker.rerank_candidates)
                    retrieved = sorted(merged.values(), key=lambda chunk: (-chunk.score, chunk.doc_id, chunk.chunk_id))[
                        :limit
                    ]
        ...
        if (
            ...
            admin_refs = self._extract_title_refs_from_query(state["query"]) or self._support_question_refs(
                state["query"]
            )
            missing_refs = self._missing_named_ref_targets(
                query=state["query"],
        ...
        if answer_type == "free_text" and _is_account_effective_dates_query(state["query"]):
            ...
            if account_ref:
                ...
                if account_results:
                    ...
                    if not family_seeds:
                        family_seeds = [
                            self._select_seed_chunk_id(account_results, seed_terms) or account_results[0].chunk_id
                        ]
            ...
            if shortlisted:
                ...
                if must_include_chunk_ids:
                    ...
                    must_include_chunk_ids = [
                        chunk_id for chunk_id in must_include_chunk_ids if chunk_id in retrieved_ids
                    ]
    ...
    async def _rerank(self, state: RAGState) -> dict[str, object]:
        ...
        if is_strict:
            ...
            if doc_ref_count >= 1 and len(retrieved) < len(retrieved_all):
                _doc_refs_lower = [str(r).strip().casefold() for r in state.get("doc_refs", []) if str(r).strip()]
                ...
                if _doc_refs_lower:
                    ...
                    _has_ref_match = any(_title_matches_any_ref(getattr(c, "doc_title", "")) for c in retrieved)
        ...
        rerank_enabled = (
            rerank_enabled_for_boolean if is_boolean else (rerank_enabled_for_strict if is_strict else True)
        )
        ...
        if str(state.get("answer_type") or "").strip().lower() == "free_text" and _is_case_outcome_query(
            str(state.get("query") or "")
        ):
            reranked = self._ensure_page_one_context(
                reranked=reranked,
            ...
            if callable(shadow_rerank) and retrieved:
                try:
                    ...
                    collector.set_selective_icr_shadow_diagnostics(
                        ...
                        provider_exit=bool(
                            getattr(self._settings.reranker, "shadow_selective_icr_provider_exit", False)
                        ),
                    )
                except Exception:
                    logger.warning(
                        "Selective ICR shadow rerank failed; continuing with primary reranker", exc_info=True
                    )
                    collector.set_selective_icr_shadow_diagnostics(
                        used=False,
                        provider_exit=bool(
