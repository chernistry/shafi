...
from shafi.core.panel_registry import PanelRegistry
...
class CompareEngine:
    ...
    def execute_common_entity(self, contract: QueryContract, doc_ids: list[str]) -> CompareResult | None:
        ...
        if not shared_entities:
            return (
                None
                if contract.answer_type.strip().lower() == "free_text"
                else CompareResult(
    ...
    def _resolve_doc_ids(self, contract: QueryContract) -> list[str]:
        ...
        source_doc_ids = [doc_id for entity in contract.primary_entities for doc_id in entity.source_doc_ids]
    ...
    def _format_attribute_compare_answer(self, contract: QueryContract, ordered_doc_ids: list[str]) -> str:
        ...
        if not ordered_doc_ids:
            return ""
        query = contract.query_text.casefold()
        entity_by_doc_id = {doc_id: entity for entity in contract.primary_entities for doc_id in entity.source_doc_ids}
