...
from shafi.core.legal_title_family import title_key
...
class PanelRegistry:
    ...
    def build_from_corpus(cls, registry: CorpusRegistry) -> PanelRegistry:
        ...
        document_entities = {
            doc_id: tuple(sorted(entity_ids)) for doc_id, entity_ids in sorted(entity_sets.items()) if entity_ids
        }
    ...
    def compare_attributes(self, doc_ids: list[str], attribute: str) -> dict[str, list[str]]:
        ...
        return {doc_id: list(self.document_fields.get(doc_id, {}).get(attribute, ())) for doc_id in doc_ids}
