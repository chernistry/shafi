...
class FieldLookupTable(BaseModel):
    ...
    def _law_fields(canonical_id: str, law: LawObject) -> dict[str, FieldAnswer]:
        ...
        FieldLookupTable._maybe_add(
            ...
            page_ids=law.field_page_ids.get(FieldType.ISSUED_BY.value)
            or law.field_page_ids.get(FieldType.AUTHORITY.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
            ...
            value=derived_authority,
            page_ids=law.field_page_ids.get(FieldType.AUTHORITY.value)
            or law.field_page_ids.get(FieldType.ISSUED_BY.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
            ...
            value=derived_date,
            page_ids=law.field_page_ids.get(FieldType.DATE.value)
            or law.field_page_ids.get(FieldType.COMMENCEMENT_DATE.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
            ...
            value=derived_date,
            page_ids=law.field_page_ids.get(FieldType.COMMENCEMENT_DATE.value)
            or law.field_page_ids.get(FieldType.DATE.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
    ...
    def _order_fields(canonical_id: str, order: OrderObject) -> dict[str, FieldAnswer]:
        ...
        FieldLookupTable._maybe_add(
            ...
            field_type=FieldType.AUTHORITY,
            value=derived_authority,
            page_ids=order.field_page_ids.get(FieldType.AUTHORITY.value)
            or order.field_page_ids.get(FieldType.ISSUED_BY.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
    ...
    def _practice_direction_fields(
        ...
        FieldLookupTable._maybe_add(
            ...
            field_type=FieldType.AUTHORITY,
            value=practice.issued_by,
            page_ids=practice.field_page_ids.get(FieldType.AUTHORITY.value)
            or practice.field_page_ids.get(FieldType.ISSUED_BY.value, []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
            ...
            page_ids=practice.field_page_ids.get(FieldType.DATE.value)
            or practice.field_page_ids.get("effective_date", []),
        )
        FieldLookupTable._maybe_add(
            rows,
            canonical_id=canonical_id,
