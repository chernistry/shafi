...
_DIFC_CASE_RE = re.compile(r"\b(CFI|CA|SCT|ENF|DEC|TCD|ARB)\s*[-/\s]*0*(\d{1,4})\s*[/-]\s*(\d{4})\b", re.IGNORECASE)
# Matches "[2025] DIFC SCT 160" — year-first bracketed format used in many private-corpus queries.
# _DIFC_CASE_RE only matches TYPE NUM/YEAR; this covers [YEAR] DIFC TYPE NUM.
_BRACKET_YEAR_CASE_RE = re.compile(
    r"\[(\d{4})\]\s+(?:DIFC\s+)?(CFI|CA|SCT|ENF|DEC|TCD|ARB)\s+0*(\d{1,4})\b", re.IGNORECASE
)
...
class QueryClassifier:
    ...
    def extract_doc_refs(cls, query: str) -> list[str]:
        ...
        # Bracketed-year format: "[2025] DIFC SCT 160" → "SCT 160/2025"
        for match in _BRACKET_YEAR_CASE_RE.finditer(normalized):
            year = match.group(1)
            case_type = match.group(2).upper()
            num = int(match.group(3))
            canonical = f"{case_type} {num:03d}/{year}"
