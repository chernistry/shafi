...
class EmbeddingClient:
    ...
    async def embed_query(self, text: str) -> list[float]:
        ...
        if self._use_local_model():
            vec = (await self._embed_local_texts([text]))[0]
    ...
    async def embed_documents(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        if not texts:
            return []
        if self._use_local_model():
            return await self._embed_local_texts(list(texts))
    ...
    def _parse_embeddings(resp: httpx.Response, *, expected_count: int) -> list[list[float]]:
        ...
        if len(embeddings) != expected_count:
            raise EmbeddingError(f"Kanon 2 API returned {len(embeddings)} embeddings for {expected_count} texts")
    ...
    async def _embed_local_texts(self, texts: list[str]) -> list[list[float]]:
        return await asyncio.to_thread(self._embed_local_texts_sync, texts)

    def _embed_local_texts_sync(self, texts: list[str]) -> list[list[float]]:
        model = self._local_model or self._load_local_model()
        encoded = model.encode(
            texts, normalize_embeddings=bool(getattr(self._settings, "local_normalize_embeddings", True))
        )
        rows: list[list[float]] = []
        for row in encoded:
            if isinstance(row, list):
                row_values = cast("list[object]", row)
                rows.append(self._coerce_local_embedding_values(row_values))
                continue
            if hasattr(row, "tolist"):
                row_values = cast("list[object]", row.tolist())
                rows.append(self._coerce_local_embedding_values(row_values))
                continue
            raise EmbeddingError("Local embedding model returned unsupported vector type")
        return rows

    def _load_local_model(self) -> Any:
        model_path = str(getattr(self._settings, "local_model_path", "")).strip()
        if not model_path:
            raise EmbeddingError("EMBED_LOCAL_MODEL_PATH is required when EMBED_PROVIDER_MODE=local")
        try:
            sentence_transformers = importlib.import_module("sentence_transformers")
        except ModuleNotFoundError as exc:
            raise EmbeddingError("sentence-transformers is required for local embeddings") from exc
        self._local_model = sentence_transformers.SentenceTransformer(model_path)
        return self._local_model

    @staticmethod
    def _coerce_local_embedding_values(values: list[object]) -> list[float]:
        coerced: list[float] = []
        for value in values:
            if not isinstance(value, (int, float, str)):
                raise EmbeddingError("Local embedding model returned non-numeric values")
