...
from shafi.core.legal_citation_parser import extract_citations, has_legal_citations
...
def load_alias_map(path: str | Path) -> dict[str, list[str]]:
    ...
    for cluster in clusters:
        raw_variants = cast("list[Any]", cluster.get("variants", []))
        variants: list[str] = [v for v in raw_variants if isinstance(v, str) and v.strip()]
        if len(variants) < 2:
            continue
...
class BM25Retriever:
    ...
    def build_index(self, chunks: list[RetrievedChunk]) -> None:
        ...
        corpus_tokens = bm25s.tokenize(corpus_texts, stopwords="english", show_progress=True)  # type: ignore[no-untyped-call]

        # Build index
        self.retriever = bm25s.BM25()  # type: ignore[no-untyped-call]
        self.retriever.index(corpus_tokens)  # type: ignore[no-untyped-call]
    ...
    def save(self) -> None:
        ...
        if self.retriever is None:
            ...
            return
        self.retriever.save(str(self.index_dir / "bm25_index"))  # type: ignore[no-untyped-call]
    ...
    def load(self) -> None:
        ...
        self.retriever = bm25s.BM25.load(str(index_path), mmap=True)  # type: ignore[no-untyped-call]
    ...
    def search(self, query: str, top_k: int = 20) -> list[tuple[str, float]]:
        ...
        # Tokenize query
        query_tokens = bm25s.tokenize(effective_query, stopwords="english", show_progress=False)  # type: ignore[no-untyped-call]
