...
class CitationGraphExpander:
    ...
    def _find_enrichment(self, chunk_id: str) -> Path | None:
        # 1. Exact match
    ...
    def _resolve(self, citation: dict[str, Any], index: dict[str, list[str]]) -> list[str]:
        """Resolve a citation dict to chunk_ids via the reverse index."""
        cited_text: str = citation.get("cited", "")
        if not cited_text:
            return []
