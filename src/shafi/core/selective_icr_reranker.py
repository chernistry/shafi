...
class SelectiveICRReranker:
    ...
    def __init__(
        ...
        *,
        ...
        self._model_name = (
            ...
            else (self._config.model_path or "selective_icr_heuristic")
        )
    ...
    def rank(
        self, query: str, candidates: Sequence[object], *, top_n: int | None = None
...
def _family_boost(query: str, document: str) -> float:
    ...
    if any(term in query for term in ("article", "section", "schedule", "provision", "clause")) and any(
        ...
        score += 0.5
    if any(
        term in query for term in ("authority", "date", "law no", "law number", "commencement", "enactment")
    ) and any(term in document for term in ("authority", "date", "law", "commencement", "enactment")):
        score += 0.4
