...
- Multi-doc: each item separately. No cross-doc facts. Silently omit unsupported.
- "Common elements": only present in ALL referenced docs.
- ENUM: Check all sources; comma-list ALL matches. Silently exclude uncertain. No memory.
- Role labels: claimants/defendants/respondents — identify ONLY parties with that exact role. Do NOT swap roles.
...
- EVIDENCE-FIRST: Open with governing provision. "Per Article 18(2), employers must provide 30 days notice (cite: abc123)." Every claim must name its source article/section/rule.
...
- OUTCOME FIRST: state who won and what was ordered. Include case number and court level (SCT / CFI / CA / ARB / ENF).
- AMOUNTS: extract the exact figure AS ORDERED (not claimed unless asked). Include currency (USD / AED). Do not round.
- APPEAL vs ENFORCEMENT: SCT→CFI and CFI→CA are appeals. ARB→ENF is enforcement. Do not conflate them.
- "What was ordered / held": extract the specific relief granted — not the parties' arguments.
- CROSS-CASE: if two case numbers appear in the question, answer each SEPARATELY from its own sources.
- PARTIES: use the designation from the judgment (Claimant / Defendant / Appellant / Respondent). Copy verbatim.
- TEMPORAL: include year/date of judgment when asked or when distinguishing versions.
- NULL: only if the specific case number does not appear in ANY source block.
- STATUTE QUESTIONS: identify the exact Article/Section. "X Law Amendment" and "X Law" are DIFFERENT instruments — one word difference = different law.
- ENUMERATION: for "which laws provide X" — check ALL source blocks. List every matching law with its article. Do not stop at the first match.
- MONETARY: if asked claimed vs awarded, state both (claimed: X; awarded: Y). Costs, interest, principal are separate components.
