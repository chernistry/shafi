...
from shafi.eval.failure_cartography_rules import (
    answers_match,
    classify_miss,
    compute_drift,
    infer_doc_family,
        ...
        for raw in cast("list[object]", payload):
            ...
            observations.append(
                RunObservation(
                    ...
                    answer_type=str(case.get("answer_type") or reviewed[qid].answer_type).strip()
                    or reviewed[qid].answer_type,
    ...
    for raw in cases:
        ...
        observations.append(
            RunObservation(
                ...
                answer_type=str(row.get("answer_type") or reviewed[qid].answer_type).strip()
                or reviewed[qid].answer_type,
...
def build_failure_ledger(
    *, reviewed: dict[str, ReviewedGoldenCase], observations: list[RunObservation]
) -> FailureLedger:
