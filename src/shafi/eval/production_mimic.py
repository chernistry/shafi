    ...
    for raw in raw_results_payload:
        ...
        if not weak_anchor:
            ...
            for doc, used_pages in used_by_doc.items():
                ...
                if (
                    wants_title
                    and 1 in retrieved_nums
                    and 1 not in used_nums
                    and doc not in {_page_doc(page) for page in minimal_set if _page_num(page) == 1}
                ):
                    weak_anchor = True
                    break
                if (
                    wants_page_two
                    and 2 in retrieved_nums
                    and 2 not in used_nums
                    and doc not in {_page_doc(page) for page in minimal_set if _page_num(page) == 2}
                ):
                    weak_anchor = True
                    break
                if wants_outcome:
                    last_page = max(retrieved_nums)
                    if last_page not in used_nums and doc not in {
                        _page_doc(page) for page in minimal_set if _page_num(page) == last_page
                    }:
                        weak_anchor = True
                        break
...
def aggregate_hybrid_strict_judge(
    *,
    ...
    def _pick_min(key: str) -> float | None:
        values = [_as_float(block.get(key)) for block in (cheap, strict) if key in block and block.get(key) is not None]
        if not values:
            return None
        return min(values)
...
def aggregate_hybrid_strict_eval(
    *,
    ...
    def _pick_min(metric: str) -> float | None:
        values = [_as_float(block.get(metric)) for block in (cheap, strict) if block.get(metric) is not None]
        if not values:
            return None
        return min(values)
...
def should_run_strict_judge(*, subject_summary: JsonDict, candidate_row: JsonDict) -> tuple[bool, str | None]:
    ...
    upper_total = _as_float(
        candidate_row.get("upper_total_estimate"), default=_as_float(candidate_row.get("strict_total_estimate"))
    )
...
def _citation_floor_failures(citation_coverage_by_answer_type: dict[str, float]) -> list[JsonDict]:
    ...
    for answer_type, floor in _CITATION_FLOORS_BY_ANSWER_TYPE.items():
        canonical = (
            "free_text_model"
            if answer_type in {"free_text", "free_text_model", "model_free_text"}
            else (
                "free_text_structured"
                if answer_type in {"free_text_structured", "structured_free_text"}
                else answer_type
            )
        )
    ...
    *,
    ...
    if citation_floor_failures:
        ...
        no_submit_reasons.append(
            "citation floor miss: "
            + ", ".join(
                f"{item.get('answer_type') or ''}<{_as_float(item.get('floor')):.2f}"
                for item in citation_floor_failures
            )
        )
    ...
    if not submit_eligibility and not no_submit_reasons:
        no_submit_reasons.append(
            "platform-like estimate does not clear the current public baseline under strict governance"
        )

    candidate_class = (
        str(candidate_row.get("branch_class") or candidate_row.get("label") or "unknown").strip() or "unknown"
    )
    return {
        ...
        "eval": {
            ...
            "citation_floor_failure_answer_types": [
                str(item.get("answer_type") or "").strip()
                for item in citation_floor_failures
                if str(item.get("answer_type") or "").strip()
            ],
