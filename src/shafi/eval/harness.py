...
from shafi.eval.sources import (
    PdfPageTextProvider,
    QdrantPageTextFallback,
    build_sources_text,
    select_used_pages,
)
from shafi.submission.common import chunk_ids_to_page_ids, coerce_answer_type
...
class EvalResult:
    ...
    def summary(self) -> dict[str, object]:
        ...
        top_10_boolean = [row for row in sorted_cases if str(row.get("answer_type", "")).strip().lower() == "boolean"][
            :10
        ]
        hallucination_suspects = sorted(
            [row for row in self.cases if _coerce_float(row.get("citation_hallucination_rate"), default=0.0) > 0.0],
        ...
        summary: dict[str, object] = {
            ...
            if citation_hallucination_rate is None
            else round(citation_hallucination_rate, 4),
        ...
        async def _eval_case(case: GoldenCase, client: httpx.AsyncClient) -> None:
            ...
            try:
                ...
                for attempt in range(_EVAL_QUERY_MAX_ATTEMPTS):
                    ...
                    answer_final_evt = next((evt for evt in sse_events if evt.get("type") == "answer_final"), None)
                    ...
                    if telemetry_event is None:
                        ...
                        if gold:
                            ...
                            # G-score must use page-level IDs to match platform scoring.
                            # Convert both gold and predicted from chunk format to page format.
                            gold_pages = set(chunk_ids_to_page_ids(list(gold)))
                            used_pages_for_g = set(select_used_pages(payload, max_pages=0))
                            g_score_value = _g_score(used_pages_for_g, gold_pages, beta=2.5)
                        ...
                        _is_no_info_answer = (
                            token_text.strip().lower().startswith("there is no information on this question")
                        )
                        if _is_null_answer or _is_no_info_answer:
                            coverage = 1.0
                            parsed_cited_ids: set[str] = set()
                        elif cited_ids and context_ids:
                            coverage = len(set(cited_ids).intersection(set(context_ids))) / max(1, len(set(cited_ids)))
            ...
            async with lock:
                ...
                if emit_cases:
                    case_record: dict[str, object] = {
                        ...
                        "grounding_g_score_beta_2_5": (None if g_score_value is None else round(g_score_value, 4)),
                        "citation_hallucination_rate": (
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--judge-out", type=Path, default=None, help="Optional JSONL output for per-case judge records."
    )
    return parser
