...
def assess_resurrection_candidate(
    ...
    *,
    ...
    signal = (
        (evidence.hidden_g_trusted_delta * 8.0) + (strict_gain * 5.0) + (platform_gain * 6.0) + (hot_overlap * 0.01)
    )
    ...
    over_penalized_score = max(
        0.0,
        ((evidence.hidden_g_trusted_delta * 10.0) + (platform_gain * 4.0) + (strict_gain * 2.0)) * conf_weight
