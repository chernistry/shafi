...
from shafi.llm.generator_case_outcome import (
...
from shafi.llm.generator_citations import (
...
from shafi.llm.generator_cleanup import (
...
from shafi.llm.generator_prompts import (
    build_system_prompt as _build_system_prompt,
)
from shafi.llm.generator_prompts import (
    generation_fallback_models as _generation_fallback_models,
)
from shafi.llm.generator_question_types import (
...
from shafi.llm.generator_titles import (
...
from shafi.llm.generator_token_usage import (
...
from shafi.models import Citation, QueryComplexity, RankedChunk
...
class RAGGenerator:
    ...
    def build_prompt(
        ...
        *,
        ...
        enable_interleaved = bool(getattr(self._pipeline_settings, "enable_interleaved_citations", False))
        system_prompt = _build_system_prompt(
            ...
            enable_interleaved_citations=enable_interleaved,
        )
    ...
    def build_structured_free_text_answer(
        ...
        *,
        ...
        if (
            any(term in q for term in ("penalt", " fine", "fine "))
            and "define" not in q
            and "refine" not in q
            and not self._is_broad_enumeration_question(question)
        ):
    ...
    def _render_context_blocks(
        ...
        *,
        ...
        is_named_multi_title_lookup = answer_kind == "free_text" and self._is_named_multi_title_lookup_question(
            question
        )
        ...
        is_named_ref_enumeration = answer_kind == "free_text" and self._is_named_reference_enumeration_question(
            question
        )
        is_company_structure_enumeration = (
            answer_kind == "free_text" and self._is_company_structure_enumeration_question(question)
        )
        use_compact_broad_payload = (
            is_registrar_enumeration or is_named_ref_enumeration or is_company_structure_enumeration
        )
        ...
        if use_extractive_compaction and not bool(
            getattr(self._pipeline_settings, "strict_types_append_citations", False)
        ):
    ...
    def _broad_enumeration_chunk_priority(cls, question: str, chunk: RankedChunk) -> int:
        ...
        if "administered by the registrar" in query_lower:
            if (
                ...
                score -= 4
        if (
            "schedule 2" in query_lower
            and "arbitration law" in query_lower
            and "schedule 2" in lowered
            and "arbitration law" in lowered
        ):
            score += 10
    ...
    def _common_elements_chunk_priority(question: str, chunk: RankedChunk) -> int:
        ...
        question_tokens = {
            token for token in _TOKEN_RE.findall((question or "").lower()) if token not in _STOPWORDS and len(token) > 3
        }
    ...
    def _is_named_amendment_question(question: str) -> bool:
        ...
        if not q or RAGGenerator._is_broad_enumeration_question(question):
            return False
        return (
            len(RAGGenerator._question_named_refs(question=question)) >= 1
            and "enact" in q
            and ("what law did it amend" in q or "what laws did it amend" in q)
        )
    ...
    def _is_named_enactment_date_question(question: str) -> bool:
        ...
        if any(
            term in q
            for term in ("what law did it amend", "what laws did it amend", "ruler of dubai", "made by the ruler")
        ):
            return False
        if any(term in q for term in ("commencement", "come into force", "effective date", "enactment notice")):
            return False
    ...
    def _is_named_translation_requirement_question(question: str) -> bool:
        ...
        if len(RAGGenerator._question_named_refs(question=question)) < 1:
            return False
        return (
            "language other than english" in q and "upon request" in q and "provide" in q and "relevant authority" in q
        )
    ...
    def _compact_slow_broad_enumeration_chunk(self, *, question: str, text: str) -> str:
        ...
        if self._is_named_reference_enumeration_question(question):
            ...
            _append_matching(
                lambda sentence: (
                    sum(1 for ref_key in ref_keys if ref_key and ref_key in sentence.casefold())
                    >= min(2, len(ref_keys))
                )
            )
    ...
    def _named_multi_title_lookup_chunk_priority(self, *, question: str, chunk: RankedChunk) -> int:
        ...
        if "administ" in query_lower:
            if _REGISTRAR_SELF_ADMIN_RE.search(text) is not None:
                ...
                score += 6
        if any(
            term in query_lower for term in ("commencement", "come into force", "effective date", "enactment notice")
        ):
    ...
    def _question_named_refs(
        ...
        *,
        ...
        for idx, ref in enumerate(refs):
            ...
            if any(
                idx != other_idx and lowered != other_lowered and re.search(rf"\b{re.escape(lowered)}\b", other_lowered)
    ...
    def _doc_group_match_score(cls, ref: str, doc_chunks: Sequence[RankedChunk]) -> int:
        ...
        if exact_position >= 0:
            ...
            if any(
                ...
                score += 250
            if any(
                "hereby enact" in re.sub(r"\s+", " ", (chunk.text or "")).strip().casefold() for chunk in doc_chunks[:2]
            ):
                score += 150
            if any(
        ...
        if ref_match is not None:
            ...
            if law_no_key in normalized_haystack:
                score = 1200
                if any(
                    "hereby enact" in re.sub(r"\s+", " ", (chunk.text or "")).strip().casefold()
        ...
        if len(ordered_ref_tokens) >= 3 and overlap < len(ref_tokens):
            ref_bigrams = [
                f"{ordered_ref_tokens[idx]} {ordered_ref_tokens[idx + 1]}" for idx in range(len(ordered_ref_tokens) - 1)
            ]
    ...
    def build_case_outcome_answer(
        ...
        *,
        ...
        for chunk in chunks:
            ...
            clauses = cls._extract_case_outcome_clauses(
                text=chunk.text or "", prefer_order_section=prefers_order_section
            )
    ...
    def cleanup_named_commencement_answer(
        ...
        *,
        ...
        if not any(
            term in question_lower for term in ("commencement", "come into force", "effective date", "enactment notice")
        ):
            return cleaned
        ...
        for ref in refs:
            ...
            for doc_id in doc_order:
                ...
                candidate_score = match_score + (
                    25 if "hereby enact" in re.sub(r"\s+", " ", (doc_chunks[0].text or "")).strip().casefold() else 0
                )
        ...
        for ref in missing_refs:
            rebuilt.append(
                f"{len(rebuilt) + 1}. {ref}: The provided sources do not contain its commencement provision."
            )
    ...
    def cleanup_named_enactment_date_answer(
        ...
        *,
        ...
        for ref in refs:
            ...
            rebuilt.append(
                f"{len(rebuilt) + 1}. {label}: The date of enactment is {enactment_date} (cite: {', '.join(cited_ids)})"
            )
        ...
        if len(refs) >= 1:
            ...
            if len(single_support_signatures) == 1:
                ...
                if len(refs) == 1:
                    ...
                    if ref_label and (clean_label.isupper() or clean_label.casefold() == ref_label.casefold()):
        ...
        for ref in refs:
            ...
            if support is None:
                if len(refs) == 1:
                    return cleaned
                rebuilt.append(
                    f"{len(rebuilt) + 1}. {ref}: The provided sources do not contain its administration provision."
                )
                continue
        ...
        *,
        ...
        for chunk in doc_chunks:
            ...
            for idx, line in enumerate(lines):
                window = line
                if idx + 1 < len(lines) and re.fullmatch(
                    r"(?:USD|US\$)?\s*[0-9,]+(?:\.\d+)?", lines[idx + 1], flags=re.IGNORECASE
                ):
                ...
                if amount_match is None:
                    continue
                # Skip year-like numbers (1900-2099) from law citations
                raw_amt = amount_match.group(1).strip().replace(",", "")
                if raw_amt.isdigit() and 1900 <= int(raw_amt) <= 2099:
                    continue
    ...
    def cleanup_named_penalty_answer(
        ...
        *,
        ...
        cleaned = (answer or "").strip()
        question_lower = (question or "").strip().lower()
        if not any(term in question_lower for term in ("penalt", "fine")) or cls._is_broad_enumeration_question(
            question
        ):
            return cleaned
        ...
        for ref in refs:
            ...
            # Deduplicate: skip if same clause already emitted (same doc resolved twice)
            clause_key = clause.strip().lower()
            ...
            for doc_id in doc_order:
                ...
                if title_chunk_id:
                    ...
                    if title_chunk is not None and cls._page_num(title_chunk.section_path) == 1:
                        title_score += 180
                    if (
                        title_chunk is not None
                        and "enactment notice" in re.sub(r"\s+", " ", (title_chunk.text or "").strip()).casefold()
                    ):
                        title_score += 80
                ...
                if "updated" in question_lower and updated_value and updated_ids:
                    ...
                    if any(
                        cls._page_num(chunk.section_path) == 1 for chunk in doc_chunks if chunk.chunk_id in updated_ids
                    ):
                        updated_score += 80
    ...
    def build_consolidated_version_published_answer(
        ...
        *,
            ...
            for ref in refs:
                ...
                for doc_id in doc_order:
                    ...
                    if match_score > best_match_score:
                        ...
                        best_label = (
                            cls._clean_structured_doc_label(cls._recover_doc_title_from_chunks(doc_chunks) or ref)
                            or ref
                        )
        ...
        if best_value and best_ids:
            return (
                f"The consolidated version of {best_label} was published in {best_value} (cite: {', '.join(best_ids)})."
            )
    ...
    def cleanup_named_amendment_answer(
        ...
        *,
        ...
        amender_label = (
            cls._recover_doc_title_from_chunks(amender_chunks, prefer_citation_title=True)
            or amendment_ref
            or "the amendment law"
        )
            ...
            for chunk in doc_chunks:
                ...
                for marker in normalized_markers:
                    if marker in normalized:
                        score += 20
                if (
                    "which specific" in question_lower
                    and "specific difc laws" in question_lower
                    and "difc law" in normalized
                ):
                    score += 2
                if score > best_score:
                    best_score = score
        ...
        *,
        ...
        if not enactment_date or generic_enactment_reference:
            for chunk in chunks:
                ...
                if (
                    refs
                    and not any(cls._doc_group_match_score(ref, [chunk]) > 0 for ref in refs)
                    and not enactment_like_chunk
                ):
                    continue
                _consume_enactment_details([chunk])
                if (
                    enactment_date
                    and enactment_date.casefold()
                    != "the date specified in the enactment notice in respect of this law".casefold()
                ):
                    break
        ...
        for doc_key in doc_order:
            ...
            if title and ruler_chunk_id and commencement_chunk_id:
                ...
                if title_key not in merged_by_title:
                    ...
                    better_title = (
                        clean_title if cls._should_prefer_extracted_title(prev_title, clean_title) else prev_title
                    )
            ...
            if target_year:
                ...
                support_has_target_year = any(
                    any(
                        ...
                        and any(
                            marker in sentence.casefold() for marker in ("hereby enact", "enacted", "made this law")
                        )
        ...
        *,
        ...
        for doc_id in doc_order:
            ...
            if not doc_chunks:
                continue
            title = cls._recover_doc_title_from_chunks(doc_chunks) or re.sub(
                r"\s+", " ", (doc_chunks[0].doc_title or "").strip()
            )
            title = title.strip(" ,.;:")
    ...
    def _compact_chunk_text(self, *, question: str, text: str, top_n: int) -> str:
        ...
        query_terms = {token.lower() for token in _TOKEN_RE.findall(question) if token.lower() not in _STOPWORDS}
        if not query_terms and not article_keys:
            return " ".join(sentences[: max(1, top_n)])

        scored: list[tuple[int, int]] = []
        for idx, sentence in enumerate(sentences):
            tokens = {token.lower() for token in _TOKEN_RE.findall(sentence) if token.lower() not in _STOPWORDS}
