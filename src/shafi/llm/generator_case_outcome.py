...
if TYPE_CHECKING:
    ...
    from shafi.llm.generator_types import CaseOutcomeCandidate
...
def clean_case_outcome_clause(clause: str) -> str:
    ...
    if "fair, reasonable and proportionate award of costs" in cleaned.casefold() and "sum of usd" in cleaned.casefold():
        ...
        if amount_match is not None:
            return (
                f"The Appellant was awarded costs in the sum of {amount_match.group(1).upper()} {amount_match.group(2)}"
            )
    if cleaned.casefold().startswith(
        "the defendant's application for immediate judgment and/or strike out was dismissed"
    ):
    ...
    if cleaned.casefold().startswith("the application was dismissed and the applicant was ordered to pay"):
        return "The Application was dismissed"
    if cleaned.casefold().startswith(
        "accordingly, the application must be dismissed, and the claimant shall bear its own costs"
    ):
