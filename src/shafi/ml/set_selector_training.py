...
class SetSelectorTrainer:
    ...
    def prepare_set_utility_data(
        ...
        *,
        ...
        historical_map = {
            question_id: list(page_ids) for question_id, page_ids in (historical_selected_pages or {}).items()
        }
        for row in rows:
            ...
            candidate_page_texts = {
                candidate.page_id: candidate.snippet_excerpt or candidate.page_id for candidate in row.page_candidates
            }
    ...
    def select_compact_evidence_set(
        ...
        *,
        ...
        artifact = _load_selector_artifact(artifact_path)
        scores = self.score_candidate_pages(
            query=query, candidate_page_texts=candidate_page_texts, artifact_path=artifact_path
        )
        query_tokens = set(_tokenize(query))
