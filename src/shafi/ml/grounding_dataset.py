...
def _build_reviewed_manifest_metadata(
    *,
    ...
    reviewed_source_paths = {
        ...
        "reviewed_page_benchmark_high_confidence_81": str(
            reviewed_dir / "reviewed_page_benchmark_high_confidence_81.json"
        ),
        "reviewed_page_benchmark_medium_plus_high_95": str(
            reviewed_dir / "reviewed_page_benchmark_medium_plus_high_95.json"
        ),
    ...
    for question_id in question_ids:
        ...
        rows.append(
            GroundingMlRow(
                ...
                doc_ref_signatures=_build_doc_ref_signatures(reference.doc_refs),
                holdout_doc_family_key=_derive_holdout_doc_family_key(
                    doc_refs=reference.doc_refs,
                    page_candidates=page_candidates,
                ),
...
def _load_raw_results(path: Path) -> dict[str, _NormalizedRawRow]:
    ...
    for item in items:
        ...
        if not isinstance(case, dict) or not isinstance(telemetry, dict):
            continue
        question_id = (
            _coerce_str(case.get("question_id"))
            or _coerce_str(case.get("case_id"))
            or _coerce_str(telemetry.get("question_id"))
        )
    ...
    if isinstance(soft_answers, list):
        for item in soft_answers:
            ...
            label_rows[question_id] = _LabelRecord(
                ...
                label_weight=_coerce_label_weight(
                    item.get("label_weight"), fallback_confidence=_coerce_str(item.get("confidence"))
                ),
...
def _build_page_candidates(
    *,
    ...
    for page_id in page_ids:
        ...
        anchor_hits = [
            anchor for anchor in hard_anchor_strings if anchor and anchor.casefold() in snippet_excerpt.casefold()
        ]
...
def _build_feature_inventory(*, rows: list[GroundingMlRow]) -> str:
    ...
    lines = [
        ...
        "- `doc_ref_signatures`, `holdout_doc_family_key`",
...
def _build_doc_ref_signatures(doc_refs: list[str]) -> list[str]:
    """Build normalized doc-ref signatures for holdout grouping.

    Args:
        doc_refs: Declared document references from telemetry.

    Returns:
        Deterministic normalized signatures.
    """
    signatures: list[str] = []
    for doc_ref in doc_refs:
        signature = _normalize_doc_ref_signature(doc_ref)
        if signature and signature not in signatures:
            signatures.append(signature)
    return signatures


def _derive_holdout_doc_family_key(
    *,
    doc_refs: list[str],
    page_candidates: list[PageCandidateRecord],
) -> str:
    """Derive a stable holdout key for doc-family evaluation.

    Args:
        doc_refs: Declared document references from telemetry.
        page_candidates: Candidate pages built for the row.

    Returns:
        Stable holdout key string.
    """
    signatures = _build_doc_ref_signatures(doc_refs)
    if signatures:
        return "||".join(signatures)
    candidate_doc_ids = sorted({candidate.doc_id for candidate in page_candidates if candidate.doc_id})
    if candidate_doc_ids:
        return "docs:" + "||".join(candidate_doc_ids[:3])
    return "unscoped"


def _normalize_doc_ref_signature(doc_ref: str) -> str:
    """Normalize a document reference into a family signature.

    Args:
        doc_ref: Raw document reference.

    Returns:
        Lowercased family signature.
    """
    tokens = [
        token
        for token in _coerce_str(doc_ref).lower().replace("/", " ").replace("-", " ").split()
        if token and not token.isdigit() and token not in {"difc", "no", "of", "the"}
    ]
    return " ".join(tokens)
