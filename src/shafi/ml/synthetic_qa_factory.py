...
from shafi.models.schemas import LegalSegment, SegmentType
...
class SyntheticQAFactory:
    ...
    def generate_all(
        ...
        *,
        ...
        examples = [
            ...
            *self.generate_temporal_questions(
                registry=registry, graph=graph, page_texts=page_texts, page_doc_ids=page_doc_ids
            ),
            ...
            *self.generate_bridge_fact_questions(
                bridge_facts=bridge_facts, page_texts=page_texts, page_doc_ids=page_doc_ids
            ),
        ]
            ...
            if commencement is not None:
                examples.append(
                    self._build_example(
                        ...
                        difficulty="medium",
                        gold_page_ids=[commencement.evidence_page_id]
                        if commencement.evidence_page_id
                        else list(law.page_ids[:1]),
                        source_object_ids=[law.object_id],
    ...
    def paraphrase_question(self, question: str) -> list[str]:
        ...
        return [
            variant.strip()
            for variant in self._paraphrase_fn(question)
            if variant.strip() and variant.strip() != question
        ]
    ...
    def validate_example_against_gold_pages(
        ...
        *,
        ...
        if example.answer in gold_text:
            return True
        return all(
            component in gold_text for component in [part.strip() for part in example.answer.split(",") if part.strip()]
        )
    ...
    def _build_example(
        ...
        *,
        ...
        return example.model_copy(
            update={"generation_method": f"{generation_method}+paraphrase", "question": variants[0]}
        )
