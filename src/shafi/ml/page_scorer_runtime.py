...
from shafi.ml.training_scaffold import SUPPORTED_FEATURE_POLICIES

if TYPE_CHECKING:
    from collections.abc import Mapping, Sequence

    from shafi.models.schemas import QueryScopePrediction, RetrievedPage
...
class RuntimePageScorer:
    ...
    def __init__(self, *, model_path: str) -> None:
        ...
        # Pre-warm the lru_cache to avoid 1s cold-start on first request.
        _load_page_scorer_bundle(self._model_path)
    ...
    def rank_pages(self, request: RuntimePageScoringRequest) -> RuntimePageScorerResult:
        ...
        # legacy_used_page_ids are the chunks selected for the answer — the closest
        # runtime proxy for training's "legacy_cited" signal (pages cited in the answer).
        legacy_used_rank_map = _build_rank_map(request.legacy_used_page_ids)
        ...
        feature_rows = [
            _build_runtime_feature_row(
                ...
                legacy_used_rank_map=legacy_used_rank_map,
...
def _build_runtime_feature_row(
    *,
    ...
    legacy_used_rank_map: Mapping[str, int],
    ...
    Args:
        ...
        legacy_used_rank_map: 1-based rank map for legacy-used (cited-proxy) pages.
    ...
    # Derive max page num for is_last_page from candidates within same doc.
    doc_page_nums = [p.page_num for p in request.page_candidates if p.doc_id == page.doc_id]
    max_page = max(doc_page_nums) if doc_page_nums else page.page_num
    return {
        ...
        "is_last_page": int(page.page_num == max_page),
        ...
        "is_strict_type": int(request.answer_type in {"boolean", "number", "date", "name", "names"}),
        "multi_doc_query": int(len(doc_rank_map) > 1),
        "is_compare_scope": int(request.scope.scope_mode.value == "compare_pair"),
        "query_token_count": len(request.query.split()),
        "answer_char_count": len(request.normalized_answer),
        # Use legacy_used_page_ids as runtime proxy for training's "legacy_cited" signal.
        # Training: cited = pages the LLM explicitly referenced.  At inference, the closest
        # available signal is the support chunks selected for the answer (legacy_used_page_ids).
        # Previously hardcoded to 0 — model importance for these features is 33 and 13
        # (6th and 12th highest), so fixing this substantially reduces train/inference skew.
        "from_legacy_cited": int(page.page_id in legacy_used_rank_map),
        "legacy_cited_rank": int(legacy_used_rank_map.get(page.page_id, 0)),
        # r3 features: interaction-friendly signals for gradient-boosted models
        "query_content_jaccard": _jaccard_similarity(request.query, page.page_text or ""),
        "page_position_ratio": int(page.page_num) / (len(request.page_candidates) or 1),
        "article_match_in_content": int(_article_match(request.query, page_text)),
        "has_definition_markers": int(_has_definition_markers(page_text)),
        "sidecar_rank_normalized": (
            int(retrieved_rank) / len(request.page_candidates)
            if retrieved_rank > 0 and len(request.page_candidates) > 0
            else 0.0
        ),
        # v4 features: article-level scorer failure targeting
        "article_ref_match": int(_article_ref_match(request.query, page_text)),
        "page_is_early_in_doc": int(page.page_num <= max(1, int(max_page * 0.3))),
        "legal_keyword_density": _legal_keyword_density(page_text),
        # v5 features: version confusion disambiguation
        "doc_pages_in_candidates": len(doc_page_nums),
        "same_page_num_competitors": sum(
            1 for p in request.page_candidates if p.page_num == page.page_num and p.doc_id != page.doc_id
        ),
        "query_law_name_in_content": int(_query_law_name_in_content(request.query, page_text)),
        "is_amendment_content": int(_is_amendment_content(page_text)),
        "query_is_temporal": int(bool(_TEMPORAL_RE.search(request.query))),
    }
...
def _predict_scores(*, model: object, feature_matrix: object) -> list[float]:
    ...
    if hasattr(model, "predict_proba"):
        ...
        rows_obj = (
            cast("_SupportsToList", probabilities).tolist() if hasattr(probabilities, "tolist") else probabilities
        )
...
# ---------------------------------------------------------------------------
# Feature helpers for r3 features (mirroring training_scaffold helpers)
# ---------------------------------------------------------------------------
...
def _jaccard_similarity(text_a: str, text_b: str) -> float:
    ...
    tokens_a = {tok.lower() for tok in _TOKEN_RE.findall(text_a)} if text_a else set()
    tokens_b = {tok.lower() for tok in _TOKEN_RE.findall(text_b)} if text_b else set()
...
def _article_ref_match(query: str, content_lower: str) -> bool:
    """Check whether a full article reference from query appears in content."""
