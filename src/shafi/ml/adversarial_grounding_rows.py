...
def build_adversarial_grounding_rows(
    ...
    *,
    ...
    return sorted(
        adversarial_rows, key=lambda item: (item.question_id, item.negative_family.value, item.adversarial_id)
    )
...
def _build_adversarial_row(
    *,
    ...
    positive_page_ids = _positive_page_ids(row)
    negative_ids = list(
        dict.fromkeys(page_id for page_id in negative_page_ids if page_id and page_id not in positive_page_ids)
    )
