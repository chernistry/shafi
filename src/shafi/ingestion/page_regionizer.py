...
class PageRegionizer:
    ...
    def extract_regions(self, page_image: RenderedPage, page_id: str) -> list[VisualRegion]:
        ...
        for block in page_image.text_blocks:
            ...
            region = VisualRegion(
                ...
                signature_data=self.extract_signature_block(block.text)
                if region_type is RegionType.SIGNATURE
                else None,
            )
    ...
    def extract_table_structure(self, region_text: str) -> TableData:
        ...
        if not lines:
            return TableData()
        split_rows = [[cell.strip() for cell in _TABLE_SPLIT_RE.split(line) if cell.strip()] for line in lines]
    ...
    def extract_signature_block(self, region_text: str) -> SignatureData:
        ...
        if not signer:
            judge_match = re.search(
                r"\b(?:judge|justice|registrar|chief justice)\b[:\s-]*(.+)", region_text, re.IGNORECASE
            )
