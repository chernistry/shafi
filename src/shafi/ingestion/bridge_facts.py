...
from shafi.models import BridgeFact, BridgeFactType

if TYPE_CHECKING:
    from pathlib import Path

    from shafi.ingestion.canonical_entities import EntityAliasResolver
    from shafi.models import LegalSegment
    from shafi.models.applicability import ApplicabilityGraph
    from shafi.models.legal_objects import CaseObject, CorpusRegistry, LawObject, LegalEntity
    ...
    def generate_all(
        ...
        *,
        ...
        for law in corpus_registry.laws.values():
            ...
            facts.extend(
                self.generate_law_definition_facts(law_obj=law, entity_resolver=entity_resolver, segments=law_segments)
            )
            facts.extend(
    ...
    def generate_case_party_facts(
        ...
        *,
        ...
        for party in case_obj.parties:
            ...
            canonical_text = _normalize_text(f"Case {case_label} involves {party.name} as {party.role or 'party'}.")
            facts.append(
                BridgeFact(
    ...
    def generate_case_judge_facts(
        ...
        *,
        ...
        for judge in case_obj.judges:
            ...
            canonical_text = _normalize_text(f"Case {case_label} was decided by {normalized_judge}.")
            facts.append(
                BridgeFact(
    ...
    def generate_case_law_facts(
        ...
        *,
        ...
        for law_title in case_obj.cited_law_titles:
            ...
            canonical_text = _normalize_text(f"Case {case_label} considers {law_title}.")
            facts.append(
                BridgeFact(
        ...
        *,
        ...
        return [
            BridgeFact(
                ...
                canonical_text=_normalize_text(f"In case {case_label}, the court outcome was: {summary}."),
        ...
        for segment in segments:
            ...
            facts.append(
                BridgeFact(
                    ...
                    canonical_text=_normalize_text(f"{law_obj.short_title or law_obj.title} defines: {excerpt}"),
