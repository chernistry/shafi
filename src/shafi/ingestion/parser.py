...
from shafi.config.settings import IngestionSettings
from shafi.models import DocType, DocumentSection, ParsedDocument, ProvidedChunk
...
class DocumentParser:
    ...
    def __init__(
        ...
        *,
        ...
        self._pdf_text_min_chars = int(
            pdf_text_min_chars if pdf_text_min_chars is not None else ingestion_settings.parser_pdf_text_min_chars
        )
        self._pdf_text_min_words = int(
            pdf_text_min_words if pdf_text_min_words is not None else ingestion_settings.parser_pdf_text_min_words
        )
    ...
    def _parse_provided_chunks(chunks_obj: object) -> list[ProvidedChunk]:
        ...
        for item_obj in cast("list[object]", chunks_obj):
            ...
            citation_values = [str(v) for v in cast("list[object]", citations)] if isinstance(citations, list) else []
