...
from shafi.models.applicability import (
    ApplicabilityEdge,
...
if TYPE_CHECKING:
    ...
    from shafi.models import ParsedDocument
...
def build_corpus_registry_from_parsed_documents(parsed_docs: Sequence[ParsedDocument]) -> CorpusRegistry:
    ...
    for doc in parsed_docs:
        ...
        if legal_doc_type not in {
            LegalDocType.LAW,
            LegalDocType.REGULATION,
            LegalDocType.AMENDMENT,
            LegalDocType.ENACTMENT_NOTICE,
        }:
            continue
...
def validate_graph(graph: ApplicabilityGraph) -> list[GraphWarning]:
    ...
    for edge in graph.edges:
        ...
        if (
            edge.edge_type
            in {ApplicabilityEdgeType.AMENDS, ApplicabilityEdgeType.SUPERSEDES, ApplicabilityEdgeType.REPLACES}
            and not edge.effective_date
        ):
            warnings.append(
                GraphWarning(
