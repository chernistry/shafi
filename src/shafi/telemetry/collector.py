...
from shafi.models import TelemetryPayload
...
class TelemetryCollector:
    ...
    def set_grounding_portfolio_diagnostics(
        ...
        *,
        ...
        self._grounding_portfolio_decision_reasons = [
            str(reason).strip() for reason in decision_reasons if str(reason).strip()
        ]
    ...
    def set_grounding_escalation_diagnostics(
        ...
        *,
        ...
        self._grounding_escalation_reasons = [str(reason).strip() for reason in (reasons or []) if str(reason).strip()]
        ...
        self._grounding_relevance_verifier_fallback_reason = str(relevance_verifier_fallback_reason).strip()
    ...
    def set_selective_icr_shadow_diagnostics(
        ...
        *,
        ...
        self._selective_icr_shadow_chunk_ids = [
            str(chunk_id).strip() for chunk_id in (chunk_ids or []) if str(chunk_id).strip()
        ]
        self._selective_icr_shadow_page_ids = [
            str(page_id).strip() for page_id in (page_ids or []) if str(page_id).strip()
        ]
    ...
    def set_request_metadata(
        self, *, question_id: str | None = None, answer_type: str | None = None, doc_refs: list[str] | None = None
    ) -> None:
    ...
    def finalize(self) -> TelemetryPayload:
        ...
        ttft_ms = total_ms if self._ttft_time is None else int((self._ttft_time - self._start_time) * 1000.0)
    ...
    def _finalize_chunk_snippets(self) -> dict[str, str]:
        ...
        return {
            chunk_id: self._chunk_snippets[chunk_id] for chunk_id in ordered_ids if chunk_id in self._chunk_snippets
        }
