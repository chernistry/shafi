...
from shafi.submission.common import load_cases, select_submission_used_pages
...
async def capture_query_artifacts(
    *,
    ...
    raw_results_path.write_text(
        json.dumps(
            [_serialize_platform_case_result(result) for result in repaired_results], ensure_ascii=False, indent=2
        )
        + "\n",
        encoding="utf-8",
    )
