...
from shafi.config.settings import build_score_settings_fingerprint
...
from shafi.submission.common import (
    SubmissionAnswer,
    SubmissionCase,
    as_int,
...
def _page_ids_to_retrieval_refs(page_ids: list[str]) -> list[dict[str, object]]:
    ...
    return [{"doc_id": doc_id, "page_numbers": sorted(page_numbers)} for doc_id, page_numbers in sorted(by_doc.items())]
...
def _result_anomaly_flags(result: PlatformCaseResult) -> list[str]:
    ...
    answer_text = (
        projected_answer
        if isinstance(projected_answer, str)
        else ("null" if projected_answer is None else str(projected_answer))
    )
    telemetry = result.telemetry
    is_unanswerable_strict, is_unanswerable_free_text = classify_unanswerable_answer(answer_text, answer_type)
    ...
    if (
        not (is_unanswerable_strict or is_unanswerable_free_text)
        and raw_support_surfaces_present
        and not projected_pages
    ):
...
async def _repair_anomalous_results(
    ...
    *,
    ...
    for index, result in enumerate(results):
        ...
        old_answer_text = (
            old_answer if isinstance(old_answer, str) else ("null" if old_answer is None else str(old_answer))
        )
        new_answer_text = (
            new_answer if isinstance(new_answer, str) else ("null" if new_answer is None else str(new_answer))
        )
...
def _build_equivalence_canary(
    *,
    ...
    for case_id, baseline in baseline_by_id.items():
        ...
        if (
            str(baseline.telemetry.get("model_llm") or "").strip()
            != str(candidate.telemetry.get("model_llm") or "").strip()
        ):
...
def _infer_source_truth_audit_path(source_submission_path: Path) -> Path:
    ...
    if match is None:
        raise ValueError(
            f"Cannot infer source truth audit scaffold path from submission filename: {source_submission_path}"
        )
...
def _resolve_source_truth_audit_path(
    ...
    path = (
        Path(raw_source_truth_audit_path)
        if raw_source_truth_audit_path
        else _infer_source_truth_audit_path(source_submission_path)
    )
    if not path.exists():
...
def _normalize_retrieved_chunk_pages(refs_obj: object) -> tuple[tuple[str, tuple[int, ...]], ...]:
    ...
    for ref in refs:
        ...
        if not doc_id or not isinstance(page_numbers_obj, list):
            continue
        page_numbers = sorted(
            {int(page) for page in cast("list[object]", page_numbers_obj) if isinstance(page, int | float)}
        )
...
def _is_anchor_comparison_question(question: str, answer_type: str) -> bool:
    ...
    return any(
        term in normalized
        for term in ("party", "claimant", "defendant", "judge", "presided", "date of issue", "issued")
    )
...
def _anchor_page_restitution_refs(
    *,
    ...
    for ref in refs:
        ...
        if not doc_id or not isinstance(page_numbers_obj, list):
            continue
        page_numbers = {int(page) for page in cast("list[object]", page_numbers_obj) if isinstance(page, int | float)}
        if not page_numbers:
            continue
    ...
    *,
    ...
    for answer in answers:
        ...
        answer_text = (
            ...
            else (answer_value if isinstance(answer_value, str) else json.dumps(answer_value, ensure_ascii=False))
        )
        results.append(
            PlatformCaseResult(
                case=case,
...
def _build_support_shape_report(scaffold: dict[str, object]) -> dict[str, object]:
    ...
    for record in records:
        ...
        flags = [
            str(flag).strip()
            for flag in cast("list[object]", record.get("support_shape_flags") or [])
            if str(flag).strip()
        ]
...
async def _download_phase_documents(client: PlatformEvaluationClient, paths: PlatformPaths, *, refresh: bool) -> None:
    has_pdf = (
        any(path.suffix.lower() == ".pdf" for path in paths.docs_dir.rglob("*.pdf"))
...
def _check_existing_artifact_preflight(
    ...
    *,
    ...
    if preflight_path is None:
        if not force:
            raise RuntimeError(
                f"No preflight_summary found near {submission_path}. Use --force-submit-existing to bypass this check."
            )
    ...
    try:
        ...
        if not force:
            raise RuntimeError(
                f"Cannot parse preflight_summary at {preflight_path}. Use --force-submit-existing to bypass."
            ) from err
    ...
    if issues:
        ...
        if not force:
            raise RuntimeError(
                f"Preflight checks failed for existing artifact: {summary}. Use --force-submit-existing to bypass."
            )
        logger.warning(
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if args.support_only_challenger:
        source_submission_path = (
            Path(args.source_submission_path) if args.source_submission_path else paths.submission_path
        )
        source_questions_path = Path(args.source_questions_path) if args.source_questions_path else paths.questions_path
        source_raw_results_path = (
            Path(args.source_raw_results_path) if args.source_raw_results_path else paths.raw_results_path
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--source-submission-path", help="Source platform submission JSON for --support-only-challenger."
    )
    ...
    parser.add_argument(
        "--source-raw-results-path",
        help="Source raw_results.json for support-only challengers that need context_page_ids.",
    )
    parser.add_argument("--artifact-suffix", help="Optional suffix for generated artifact filenames.")
    parser.add_argument(
        "--refresh-downloads", action="store_true", help="Redownload questions/documents for the active phase."
    )
    parser.add_argument(
        "--skip-ingest", action="store_true", help="Reuse the existing phase-specific Qdrant collection."
    )
    parser.add_argument(
        "--submit", action="store_true", help="Upload submission.json and code_archive.zip to the platform."
    )
    parser.add_argument(
        "--submit-existing",
        action="store_true",
        help="Upload an existing submission.json and code archive without downloading, ingesting, or querying.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--query-concurrency", type=int, help="Override question execution concurrency for artifact generation."
    )
    parser.add_argument(
        "--equivalence-canary-concurrency",
        type=int,
        help="Optional secondary concurrency to compare against the primary artifact build.",
    )
